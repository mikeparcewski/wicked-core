...
- **Status:** rev 13, ready for review. **No open operator items.** The operator decided all three design questions (2026-09-23, §8.3–§8.8) and on 2026-09-24 confirmed the two remaining recommendations: the floor-override rule (§8.5) and "team runs never use `creator_seat`" (§8.1). Every decision is written into the text it governs.
- **Date:** 2026-09-23
- **Rev 13 (2026-09-26): X1 — the PA scopes a plan that declares no scope (operator decisions, 2026-09-26).** A migration survey found that rev 4's "no declared scope ⇒ 100" rule put EVERY preset launch in the top band: a preset has no `touch`, so `intent_score_for_run` / `assess_intent` scored any plan with a creator step at 100 — the full floor, high risk, and a `plan_approval` pause even in auto mode, the default `feature` launch included. Work with no repo (documents, demos, RFP content) always scored 100 too. So the §11.3 rows that said "None" for creator consumers were wrong on `main`. Decided and built (seam X1):
  1. **A plan with a creator step and no declared `touch` is scored by the PA** — every preset launch, and a user plan that leaves `touch` out. Its rev 1 is one READ-ONLY step, `pa-scope` (the catalog `understand` entry on the PA seat), accepted by the engine without the approval matrix (no creator step, so nothing it does can change the tree; the run-level policy never pauses it). The PA grounds through the garden skills / CLI shim as every worker does, and ends its answer with ONE line of a fixed grammar, parsed like `PLAN+`: on a repo, `SCOPE {"touch":["src/auth/login.rs", …]}`; with no repo, `RISK {"score":N,"reasons":["…"]}`.
  2. **At the scope step's boundary** the launch plan (the scope step first, then its own steps, with the declared touch) goes through the ONE pipeline (§8.4) as the run's INITIAL plan: `plan.proposed{by:<PA seat>, kind:"initial", source: its understand turn}` → `path.scored{basis:"intent"}` → floor fill → compose → the approval matrix (§8.6) → `plan.accepted{by:"engine"}` or a `plan_approval` gate — before any creator step dispatches. A repo run's touch set is scored by the SAME scorer and blast-radius rules as a user plan's declared touch (no usable graph still ⇒ 100). A repo-less run's deterministic baseline is the lowest band (`THRESHOLDS.repo_less_baseline` = 0), and the PA's rating can only RAISE it: it rides `path.scored.model.add`, and no graph is read.
  3. **Fail closed:** a missing or malformed answer scores 100 with the first reason `"the PA declared no scope"`. Several valid lines only make it riskier (the union of the touch sets, the highest rating). A refused or unplannable scoped plan fails the run; it never runs on with only its scope step.
  4. **Consequences:** a user plan WITH a touch set keeps rev 12's behaviour (scored at launch). The old "no declared scope ⇒ 100" rule no longer reaches any launch plan; it survives only as the fail-closed default where no PA can scope (a gate edit, before any unit ran, that drops the launch's touch). A floor override now needs a declared touch set (its floor is scored from it), else the launch refuses it. `Core.previewPlan` of a plan the PA will scope answers `graph: "pending_pa_scope"` with the baseline floor and a "pending the PA's scope" reason, never 100 as if final. A mid-run edit while the PA is scoping is refused (propose it once the plan is decided, or at its gate). T4's diff re-score still ratchets from the scoped band. It runs only on a teamed run, so on an UN-TEAMED repo run (no bus) a declared scope could never be corrected: its `SCOPE` fails closed at 100 (`"no diff re-score on an un-teamed run"`); a repo-less `RISK` has no diff to correct and stands. Only the PA seat's answer counts. A run whose scope step folded but whose scoped plan did not land (a restart in between) is never finalized: the re-drive routes it through the scope step's boundary. §8.2, §8.4, §8.5 and §11.3 below carry the corrections.
- **Rev 12 (2026-09-25): C1 built (#615); pin swaps banned.** The operator banned validator-pin swaps in plan steps, and building C1 corrected five §8.3/§10 facts against the code:
  1. **Pin swaps banned (operator, 2026-09-24).** `validator_pin` is set-only-if-unset: a step may add a pin to an unpinned entry; on a pinned entry, restating its pin is a no-op, a different pin is refused `pin_changed` (even an approved one), and `null` is refused `pin_removed`. The one shipped swap (domain-extraction/coverage, `test` entry + `COVERAGE_VALIDATOR_PIN`) becomes a **13th entry, `domain_coverage`**, that carries the coverage pin as data; the mapping restates it, so no step swaps and `BINARY_PINNED_PHASES` (`src/domain_extraction.rs:114`) stays satisfied.
  2. **The full step-field table is code:** `plan::STEP_FIELD_RULES` (`src/plan.rs`), one `apply_step` arm per row; §8.3's list below now matches it.
  3. **`deliver`** has `executes_code: false`, and today's composed deliver phase carries `EVIDENCE_FLOOR_PIN`. The entry is unpinned; the deliver step ADDS the pin (allowed on an unpinned entry), matching (`wicked-crew packages/crew/src/core/deliver.ts:798`, `:805`); rev 11's row had both wrong.
  4. **`verified_evidence`** exists on `PhaseDef` but was missing from §8.3. Only `test` and `domain_coverage` set it (`domain_coverage` preserves today's coverage phase). Composed qe-verify and deliver steps carry `false` where today's phases carry `true`; its only reader is the registration check that a `verified_evidence` phase must carry a pin, so this is a recorded difference with no runtime effect.
  5. **`run`** has no fixed kind: the entry defaults to `recon` and a step sets its own. §10's `src/pipeline.rs:236-244` "arming" citation was stale (those lines are a comment; the arming was removed earlier) and now points at the catalog. `compose` names its def `plan` (`COMPOSED_DEF_ID`); the registrar renames it to `"<run>:plan-<rev>"`.
...
- A PA-composed run's first unit is the catalog's `understand` step on the PA seat (§8.3). A user plan that declares a touch set is scored from it at launch (§8.4). **(Rev 13, X1) One rule for every author:** a plan with a creator step and no declared `touch[]` (every preset, and a user plan that left it out) is scored by the PA. Its first unit is the read-only `pa-scope` step on the PA seat. On a repo run its `SCOPE {"touch":[…]}` line is scored exactly as a declared touch set; with no repo its `RISK {"score":N,"reasons":[…]}` line raises the lowest-band baseline. The plan gate opens (or auto-passes) at that step's boundary, before any creator step. A missing or malformed answer scores `no_graph_score` = 100 with the reason `"the PA declared no scope"`: band 70–100, the full floor, high risk, and so plan approval even in auto mode. *(Revs 4–12 scored every such plan 100 at launch with `"no declared scope"`, which put every preset launch in the top band.)* A plan with no creator step and no `touch[]` scores 0. Its required deliverable is the **plan block** (§8.4): the predicted touch set, the phases the PA wants beyond the floor, and its asks.
...
**The entries**: thirteen, the minimum set that covers every shipped workflow and every crew-registered def. §11.2 maps every current phase onto them.
...
| `test` | test | evaluator | `{"human_confirm_if":"verdict_not_pass"}` | execution | `EVIDENCE_FLOOR_PIN` | false | agent | feature/test, bug/verify, migration/verify |
| `domain_coverage` | test | evaluator | `{"human_confirm_if":"verdict_not_pass"}` | execution | `COVERAGE_VALIDATOR_PIN` | false | agent | domain-extraction/coverage (the coverage judge; its pin is entry data so no step swaps a pin, rev 12) |
...
| `run` | recon (default; a step sets its own) | neutral | auto | value | — | (per step) | tool | onboarding/{index, annotate}, domain-extraction/domain-graph, qe-author-tests/verify |
| `deliver` | build | neutral | auto | execution | — (the `deliver` step adds `EVIDENCE_FLOOR_PIN`) | false | tool | crew's composed deliver phase (`packages/crew/src/core/deliver.ts:789`, appended by `composeDeliverWorkflow` `:859`) |
...
- `instructions`: set only if the entry has none (no catalog entry carries instructions today, so every mapping is unaffected).
...
- `validator_pin`: **set only if the entry has none** (operator, rev 12). A step may add a pin to an unpinned entry (approval still enforced at attach, `src/pipeline.rs:246-258`); on a pinned entry, restating the pin is a no-op, any other pin is refused `pin_changed`, and `null` is refused `pin_removed`. That is why domain-extraction/coverage uses its own `domain_coverage` entry, and qe verify and deliver add the evidence floor to unpinned entries.
...
- `skill_ref`, `allowed_skills`: set only if the entry has none (`skill_ref_changed`, `allowed_skills_changed`); that is what keeps `security_review` on the security specialist.
- `required_deliverables`: may only grow (`deliverable_removed`).
- `depends_on`: free (the plan's own order, validated by the registry).
...
Validation is `deny_unknown_fields` (`src/workflow.rs:640`) plus these rules, encoded once as `plan::STEP_FIELD_RULES` (Fixed / RunOnly / TightenOnly / SetIfUnset / ToolOnly / Free) with a test that fails if any `PlanStep` field is unclassified or any loosening passes. It runs in `plan::compose(catalog, &PlanSteps) -> Result<WorkflowDef, PlanRefusal>`, the only constructor of a per-run def.
...
- **User-composed:** the launch carries `plan: {steps:[…], touch?:[…]}` (crew `POST /runs` gains `plan`; studio's phase picker produces it). The plan is part of the **launch command** (`POST /api/v1/runs` → `Core::launch_run`, §4.0). The engine validates it and then publishes `plan.proposed{by:"human", kind:"initial"}`, before any unit runs. Crew publishes nothing. The plan runs its own steps; `understand` runs only if the user included it. The intent score comes from the plan's declared `touch[]`. **(Rev 13, X1) A plan that has any creator step (`build` or `produce`) and a missing or empty `touch[]` is scored by the PA** (§8.2): the run's first unit is the read-only `pa-scope` step, and the plan is decided from its `SCOPE` / `RISK` answer at that step's boundary. A missing or malformed answer fails closed at 100 (`"the PA declared no scope"`, S4's fail-closed rule). A floor override needs a declared `touch[]`, because the floor it removes from is scored from it. Only a plan with **no** creator step may score 0 on an empty `touch[]` at launch. The diff re-score (§8.7) ratchets the floor from there, and the PA may add phases at any of its step boundaries as a `plan.revised`, so a user plan is never silently replaced.
- **Preset:** the launch carries `preset: "<name>"`. The engine expands the preset into `steps[]`. A preset is **only** a saved phase selection, a named `steps[]` with no `touch`. So (rev 13, X1) a preset with a creator step is scored by the PA: rev 1 is the `pa-scope` step, and at its boundary the engine publishes `plan.proposed{by:<PA seat>, kind:"initial", preset:"<name>", touch:<declared>}` for the scope step plus the preset's steps. A read-only or tool-only preset is decided at launch as before (`plan.proposed{by:"human", preset:"<name>"}`, score 0).
...
Rules:
- `understand` is not a floor item. For a PA-composed plan it is ord 1, because it is how the score exists. **(Rev 13, X1)** The same holds for every plan the PA scopes (a creator plan with no declared `touch[]`, which includes every preset): its `pa-scope` step (catalog `understand`, read-only) is ord 1 and plan rev 1. The band, and so the floor and the high-risk rule, is the one its answer scores; it is the fail-closed 100 only when the answer is missing or malformed. Repo-less work starts from the lowest band (`repo_less_baseline`), raised only by the PA's `RISK` rating.
...
| The evidence-floor and coverage pins as catalog data | `src/catalog.rs` (the `build`/`test`/`review`/`security_review` entries and `domain_coverage`; qe-verify and deliver add the floor as a step field); `src/builtin_floors.rs:109` | the pin on the `test` / `build` / `review` catalog entries | move (seam C1) |
...
- **(Rev 13, X1) PA scoping, for every consumer whose preset has a creator step (`build` / `produce`).** A preset declares no `touch`. On `main` before X1 such a run therefore scored 100 (`"no declared scope"`), took the 70–100 floor and paused `plan_approval` even in auto mode, so the rows below that said "None" were wrong for every creator consumer. Now the run's first unit is the PA's read-only `pa-scope` step: one extra unit at ord 1, with the preset's steps from ord 2, so a `before:<n>` posture names an ord one later than on a pre-X1 run. On a repo the PA declares `SCOPE {"touch":[…]}` and the plan is scored against the repo's graph like a declared touch set. With no repo (documents, demos, RFP content) it rates `RISK {"score":N,"reasons":[…]}` from the content and audience, client-facing counting higher, which raises the lowest-band baseline. The floor, the high-risk rule and the §8.6 matrix then apply to that score at the scope step's boundary, before any creator step: a small, clear change proceeds in auto mode, high risk pauses, and manual mode always pauses (once: the run-level policy never pauses the scope step itself). A missing or malformed answer fails closed at 100 and pauses.
...
|---|---|---|
| feature | **Floor fill** may add phases by band (e.g. band 70–100 adds `test_plan`, `architecture`, `security_review`). **`test` and the final review move to an evaluator seat** (role neutral → evaluator), so the fence places them off the creator. The final review loses no pin (it maps to the unpinned `critique`). **PA scoping (rev 13, X1):** the PA's `pa-scope` step (ord 1) declares the touch set the plan is scored from, so a small, clear change proceeds in auto mode; the preset's own `clarify` runs after the plan gate. **Manual mode:** the `plan_approval` pause comes after the scope step, before `clarify`; `clarify`'s own raised gate (`"def"`) still pauses after it (rev 12's "one pause when the two coincide" assumed the plan gate sat at the clarify boundary). | Gate ladder and pins on build / adversarial-review / test; deliver gate; evidence bundle shape. core `tests/p10_methodology.rs`, `tests/governed_floor_and_fence.rs`, `tests/p2_gates.rs`, `tests/p14_gate_phase.rs`, `tests/dead_seat_gate.rs`; crew `tests/deliver-launch.test.ts`, `tests/deliver-phase.test.ts`, `tests/deliverable-floor-launch.test.ts` |
| bug | **Floor fill** adds `review` at band ≥ 20 (bug has none today). **PA scoping (rev 13, X1):** a `pa-scope` step at ord 1. **Manual mode:** a new `plan_approval` pause after the scope step, before triage. | reproduce → fix → verify order, the fix sweep instructions, verify's `human_confirm_if` gate and pin. The same core tests; crew `tests/deliver-launch.test.ts` |
| migration | **cutover and cleanup gain the evidence-floor pin and the creator role.** **cleanup → `build` (decision, 2026-09-24):** it removes the old path, so it is code work. It gains a worktree (`executes_code`), the evidence pin and the creator role. That is the stricter and correct classification, and a **known behaviour change**: a cleanup that passed ungated today can now be denied by the evidence floor, and it runs on a creator seat, so its judge is a distinct seat. A new `understand` ord 1 is added: the PA's `pa-scope` step (rev 13, X1), whose declared touch set scores the plan. Floor fill applies. | cutover's **unconditional** human gate; verify's gate and pin. core `tests/p14_gate_phase.rs`, `tests/governed_floor_and_fence.rs` |
...
| capture-learnings | Beyond the common changes (produce ≡ its current build/creator/value phase): **PA scoping.** It is a creator preset on a repo, so a `SCOPE` step runs first and the band is the declared touch set's. Rev 12 said "None"; on `main` it scored 100 and paused. | skill_ref on every phase; `delivery:'none'`. crew `tests/capture-learnings-skill-ref.test.ts` |
| memories | produce ≡ store. **PA scoping** (a creator preset): `SCOPE` on a repo, `RISK` without one. Rev 12 said "None"; on `main` it scored 100 and paused. | — (no dedicated test today; M-seam adds a preset launch test) |
| domain-graph-slice | validate maps to `critique`: identical controls. **PA scoping** before the plan gate (a creator preset: extract → `produce`). | — (M-seam adds a preset launch test) |
| domain-extraction | **extract's kind recon → build** (studio's stage badge changes; `StageKind`, `src/domain.rs:630`). coverage keeps `COVERAGE_VALIDATOR_PIN`, skill_ref and a worktree. **PA scoping** (rev 13, X1: a creator preset), `SCOPE` first. | core `tests/domain_extraction_e2e.rs`, `src/domain_extraction.rs:379` (drop-in test, rewritten as a preset test) |
| steering-author | **propose's kind recon → build** (badge). Gate kept (raised to `human_confirm`). **PA scoping** (rev 13, X1: a creator preset), before the plan gate. | crew `tests/governance-steering-routes.test.ts` |
| collab | **propose and revise kind recon → build** (badge). **PA scoping** (rev 13, X1: a creator preset), before the plan gate. | core `collab_def` unit tests (moved to a preset fixture) |
| interactive-* (5) | Auto-gated, unpinned, no code, the same kinds. Crew stops registering them at boot and stops seeding `$WICKED_WORKFLOWS_DIR`. **PA scoping, repo-less:** each is a `produce` preset with no repo, so a `RISK` step runs first. The PA rates the document's content and audience (client-facing higher) over the lowest-band baseline, so a routine internal draft proceeds in auto mode and a high-stakes one pauses. Rev 12 said "None"; on `main` every one scored 100 and paused. | crew `tests/interactive-chat-events.test.ts`, `tests/interactive-draft-events.test.ts`, `tests/interactive-draft-skill.test.ts`, `tests/interactive-edit-events.test.ts`, `tests/interactive-demo-events.test.ts` |
| qe-author-tests | **PA scoping** (author → `build` on a repo): a `SCOPE` step runs first and the band is the declared touch set's. Rev 12 said "None"; on `main` it scored 100 and paused. | crew `tests/qe-author-workflow.test.ts`, `tests/testing-author-route.test.ts`, `tests/testing-routes.test.ts` |
