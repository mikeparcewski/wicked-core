...
- **Rev 12 (2026-09-25): C1 built (#615); pin swaps banned.** The operator banned validator-pin swaps in plan steps, and building C1 corrected five §8.3/§10 facts against the code:
  ...
  3. **`deliver`** has `executes_code: false` and carries `EVIDENCE_FLOOR_PIN`, as today's composed deliver phase does (`wicked-crew packages/crew/src/core/deliver.ts:798`, `:805`); rev 11's row had both wrong.
...
| `deliver` | build | neutral | auto | execution | `EVIDENCE_FLOOR_PIN` | false | tool | crew's composed deliver phase (`packages/crew/src/core/deliver.ts:789`, appended by `composeDeliverWorkflow` `:859`) |
...
| The evidence-floor and coverage pins as catalog data | `src/catalog.rs` (the `build`/`test`/`review`/`security_review`/`deliver` entries and `domain_coverage`); `src/builtin_floors.rs:109` | the pin on the `test` / `build` / `review` catalog entries | move (seam C1) |
