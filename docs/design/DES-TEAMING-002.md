...
- **Rev 11 (2026-09-24): T0 amended.** The T0 build found three facts about the code that rev 10's T0 did not account for. Handing every engine `WICKED_BUS_DB` as written would have broken governed runs, so T0 now also changes the engine (wicked-core#613, wicked-crew#671):
  1. **`WICKED_BUS_DB` alone switched the gate judge to the bus.** `run_unit_and_judge_with_roster` sent every approved-validator judge to `bus_request_agent_verdict` whenever the variable was set (`src/cli_runner.rs:912-927` at `e8ae661`). That path publishes `wicked.gate.eval.requested` and denies fail-closed after 180 s (`:93`, `:446-462`). The only responder is `scripts/gate_eval_daemon.py`, which is started by hand (`tests/gate_eval_daemon_verdict.rs:4-9`). The built-in feature/bug/migration Evaluator phases all carry the evidence-floor pin (`workflows/feature.json:58`, `bug.json:55`, `migration.json:39`). So every default daemon would have stalled and denied them. **Now:** the bus judge switches on exec mediation's own setting, `WICKED_BUS_EXEC` together with `WICKED_BUS_DB`, never on the bus alone.
  2. **The actor opened the bus at startup.** The launch bridge's start-point snapshot (`src/bus.rs:707-709`) and `init_exec_consumers` (`src/cli_runner.rs:1596`, `:1608`) ran on the actor thread (`actor::run`, spawned at `src/lib.rs:520-521`), so a locked bus stalled the first command for the 5 s busy timeout. **Now:** a bus thread opens the bus and takes the snapshot. The actor never opens the bus, at startup or after. The snapshot becomes a time floor taken in `Core::spawn*` (`BusDb::floor_before`), so a request emitted after `spawn` returns is still delivered. Exec mode's actor still waits for its bus thread's answer, which keeps its arm-before-first-command and redrive-before-orphan-report contract unchanged.
  3. **One bus file, two SQLite libraries in one process.** This is the F-E2E-021 lock-loss class: closing any descriptor drops the process's POSIX locks, and one library cannot see the other's. **Rule now:** each library holds one long-lived connection to the bus file per process and never opens and closes one. The engine uses `BusDb::shared` (a process-wide handle, never closed); a source-guard test fails the build on any production `BusDb::open`. Crew uses one long-lived crew bus handle, opened at boot before the engine spawns; the project activity feed's per-request open/close now goes through it. Shutdown order: the engine's connection closes only at process exit. Crew hands the bus only to an engine that carries the rule (`Core.busConnectionStats`); an older engine gets the pre-T0 handoff.
...
- (f) the actor starts and serves commands while the bus file is locked (no 5 s stall);
