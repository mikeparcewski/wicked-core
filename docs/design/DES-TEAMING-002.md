...
- **Rev 11 (2026-09-24): T0 amended.** The T0 build found three facts about the code that rev 10's T0 did not account for. Handing every engine `WICKED_BUS_DB` as written would have broken governed runs, so T0 now also changes the engine (wicked-core#613, wicked-crew#671):
  1. **`WICKED_BUS_DB` alone switched the gate judge to the bus.** `run_unit_and_judge_with_roster` sent every approved-validator judge to `bus_request_agent_verdict` whenever the variable was set (`src/cli_runner.rs:912-927` at `e8ae661`). That path publishes `wicked.gate.eval.requested` and denies fail-closed after 180 s (`:93`, `:446-462`). The only responder is `scripts/gate_eval_daemon.py`, which is started by hand (`tests/gate_eval_daemon_verdict.rs:4-9`). The built-in feature/bug/migration Evaluator phases all carry the evidence-floor pin (`workflows/feature.json:58`, `bug.json:55`, `migration.json:39`). So every default daemon would have stalled and denied them. **Now:** the bus judge follows exec mediation, never the bus alone. It uses the bus exec mediation resolved for the unit (the cli-runner's own bus, or the actor's armed publisher's), which covers the env-free `Core::spawn_with_engine_exec` entry. Failing that, it uses `WICKED_BUS_EXEC` together with `WICKED_BUS_DB`.
