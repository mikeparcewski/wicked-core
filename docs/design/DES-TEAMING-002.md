...
- **Rev 12 (2026-09-25): C1 built (#615); pin swaps banned.** The operator banned validator-pin swaps in plan steps, and building C1 corrected five §8.3/§10 facts against the code:
  ...
  4. **`verified_evidence`** exists on `PhaseDef` but was missing from §8.3. Only `test` sets it. Composed qe-verify and deliver steps carry `false` where today's phases carry `true`; its only reader is the registration check that a `verified_evidence` phase must carry a pin, so this is a recorded difference with no runtime effect.
