...
- **Status:** PROPOSED (rev 9). **One item is PENDING an operator decision: Q4 (§13).** Until it is answered, the "unaccepted" extension of §6.3 (the unanswered and not-delivered arms of item 2) and acceptance #16 (h)–(i) are **not approved for build**. Builders implement the ruling as given (declined-and-held → council). Everything else in S2, S3 and S6 is unblocked.
...
- **Rev 5 (2026-09-23):** review on #604 at `0493046`. (1) The ruling is **extended** (status: Q4, §13): every **unaccepted** HIGH (declined-and-held, unanswered, or never delivered) is unresolved and takes the council path, so `combine_verdict(true, None)` can no longer approve one. (2) `findingId` now includes a stable location anchor, so two identical hazardous lines in one file are two findings; a secondary `lineKey` keeps moved-line correlation (§4.6).
...
- `question`: "The worker did not accept this HIGH finding (declined it / gave no answer / never received it) and the monitor holds it. Should the run continue autonomously with the work as submitted? YES = continue. NO = a human must decide."
