...
- **Status:** PROPOSED (rev 13). **No item is pending an operator decision.** Q4 (§13) was decided by the operator on 2026-09-24: an unresolved HIGH is every **unaccepted** HIGH the monitor holds — declined, unanswered, or never delivered (§6.3 item 2) — and it takes the council path (§6.7). Every clause and acceptance item in S2, S3 and S6, including #16 (h)–(i), is approved for build.
...
- **Rev 13 (2026-09-24):** operator ruling on Q4 (§13): **confirmed**. A HIGH the worker never answered, or never received (`delivery:"not_delivered"`), counts as unresolved exactly like a declined-and-held one and takes the same council path (YES → continue autonomously; NO or no verdict → human pause, gate kind `team_dispute`). Every `[Q4-pending]` clause is collapsed into one approved sentence (§3, §4.5, §4.7 step 5 and budget expiry, §6.3, §6.7, §9, §11, acceptance S2 #8, #11, #15, #16 h/i); the header no longer names a pending item.
- **Date:** 2026-09-23
...
In the hold round (§4.7 step 5, S3-owned), a monitor answers `HOLD <findingId> — <reason>` or `WITHDRAW <findingId> — <reason>` for each **unaccepted** finding (declined, unanswered, or never delivered). A review batch never asks for or accepts these.
...
3. **Confirm against the settled tree:** `git cat-file -p <T_k>:<path>` exists, its line `line` equals `evidence` **exactly** (byte for byte — the prompt asks for the exact line and `monitorFinding.evidence` IS that line; whitespace normalization serves only the finding id and the moved-line `lineKey` below), and `evidence` is at most 512 B (a longer quote is refused here, never confirmed and then truncated). Else `unconfirmed`. This is the mechanical "file:line or it did not happen" rule: a finding that cites a line that is not there never surfaces. `inDiff` is recorded as whether `path` is in the baseline..`T_k` name-status list. Out-of-diff findings are allowed (a caller the change broke) and labelled.
4. **Dedup, two keys.**
...
5. **(S3)** **Hold round.** For each monitor with ≥1 **unaccepted** finding (declined, unanswered, or never delivered), send one more turn on the same warm session. It lists each such finding with the worker's decline reason, or its state ("no answer", or "not delivered — <adviceDelivered outcome or carrier>"), and asks for `HOLD <id> — <reason>` or `WITHDRAW <id> — <reason>`, then `DONE`. No reply for an id counts as HOLD (§6.3). Monitors whose findings were all accepted or superseded get no turn.
...
- `question`: for a `declined` finding, "The worker declined this HIGH finding and the monitor holds it. Should the run continue autonomously with the worker's refusal standing? YES = continue. NO = a human must decide."; for an unanswered or undelivered finding, "The worker did not accept this HIGH finding (gave no answer / never received it) and the monitor holds it. Should the run continue autonomously with the work as submitted? YES = continue. NO = a human must decide."
- `positions`: `[{by:"worker <seat>", position:"YES — the refusal stands", reason:<the DECLINE reason>}, {by:"monitor <seat>", position:"NO — the finding stands", reason:<HOLD reason, or "no reply (counted as hold)">}]`. For an unanswered or undelivered finding the worker's `reason` is `"no answer"` or `"not delivered — <outcome>"`; its position is still YES (the work as submitted stands), and the council then judges the finding on the evidence alone.
...
- **Extension of the ruling (review on #604 at `0493046`, HIGH) — confirmed by the operator on 2026-09-24 (Q4, §13).** The operator's original words were "declined by the worker, held by the monitor". Read literally, a HIGH that was **not delivered** (non-steering carrier, a `turn_ended`/`refused` steer) or that the worker left **unanswered** was never unresolved, never reached a council, and with the judge skipped `combine_verdict(true, None)` approved it. The same fail-closed reasoning the ruling applies to a missing council verdict applies here: an unaccepted HIGH is unresolved whatever the reason it was not accepted (§6.3). So **every** HIGH that is not `accepted`, `withdrawn` or `superseded` and that the monitor holds takes the council path: YES → continue, NO or no verdict → human pause. This is the approved reading; acceptance #16 (h)–(i) test the unanswered and not-delivered arms.
...
|---|---|
| **C** changed symbols | `signals_from_diff` reads each file's paths from its `---`/`+++` headers (git-unquoted; `/dev/null` = new or deleted) and from `rename from`/`rename to` for hunk-less renames — the `diff --git` line only delimits files — and keeps, per non-docs file, the base-side lines each hunk touches (removed lines, and both neighbours of an insertion). A graph node whose span contains a touched line is in C; when none does (a pure rename/move, a mode change, lines outside every symbol) the file's node is the changed symbol, so its importers count. Only a touched file the graph does not know (a new file) counts as one changed symbol nothing reaches. |
...
8. **A wrapped unit gets the final-pass ledger.** A unit executed by `WrappedCliStepRunner` (a mock CLI binary that writes a tree-changing code diff into the worktree and exits 0), with S4 answering `monitors: 1` and a stub monitor session that returns one confirmable `FINDING`: the run emits **no** `unitCheckpoint` and **no** `adviceDelivered`; after the unit returns it emits `monitorAttached{status:"attached"}`, one `monitorFinding`, and a `teamLedger` with `monitors[0].batches == 1`, `finalPass:"completed"`, and the finding `delivery:"not_delivered"`. That finding is `unanswered` after the worker's turn (never received, §6.3) and therefore unaccepted, so the stub monitor gets exactly one hold-round turn listing it as "not delivered — <outcome>" (§4.7 step 5); the stub answers `WITHDRAW`, so the ledger records `status:"withdrawn"`, `monitorReply:{kind:"withdraw", …}`, `dispute:null`, and `teamPause:false`, and the unit continues with `gateDecided` + `unitDone`. The judge prompt contains the rendered ledger. The same fixture with the stub answering `HOLD`, or giving no reply, makes the finding unresolved and takes the §6.3 council path; that is #16 (i). The same test on the in-process path and on the bus path (`StepInput` rebuilt at `src/cli_runner.rs:1971`) produces the same ledger. Under `spawn_with_engine` with no supervisor installed, the same unit produces no team events and `UnitEvidence.team` is `None`.
...
11. Final output lines `ADVICE f-…: DECLINE — campaign.rs:325 documents the exclusion` and `ADVICE f-…: ACCEPT — added AbortController` produce one `workerAdviceResponse` each, with the matching disposition and reason. A delivered id with no line ends `unanswered`. **Hold round:** only monitors that authored an unaccepted finding get a hold-round turn, and it lists exactly those ids, each with its state: `declined` with the worker's decline reason, `unanswered` with `delivery:"injected"` as "no answer", and `unanswered` with `delivery:"not_delivered"` as "not delivered — <outcome>". A monitor whose findings are all `accepted`, `withdrawn` or `superseded` gets no turn. A missing reply for an id records `monitorReply: {kind:"hold", reason:"no reply (counted as hold)"}`. The S2 final review batch prompt contains no decline text.
...
15. **Council trigger:** a HIGH the worker did not accept and the monitor held convenes exactly one council. Its input carries the finding, the worker's position (the decline reason, "no answer", or "not delivered — <outcome>"), the monitor's reason, the `T_final` hunk at `path:finalLine` and the criterion, and it excludes the creator and the authoring/corroborating monitors. Any one of HIGH / unaccepted / held removed convenes none; an `accepted`, `withdrawn` or `superseded` HIGH, a MEDIUM, or a monitor `WITHDRAW` convenes none. A monitor with no hold-round reply for an unaccepted HIGH counts as held and convenes one. The declined, unanswered and not-delivered arms each convene one (tests #16 b, h, i).
16. **Continue or pause (§6.7).** Fixture: a teamed unit whose floor passes, whose evaluator passes and whose ledger holds one unresolved HIGH. The council result is injected through a stub of S5's entry point.
    ...
    (h) **Unanswered:** the same fixture with the HIGH `delivery:"injected"`, no `ADVICE` line, and the monitor holding (or silent): council NO and every no-verdict reason pause exactly as (b)–(d); council YES continues as (a). With `judge_skipped` set, there is never a `unitDone` without a council YES.
    (i) **Not delivered:** the same fixture on a bridge that does not advertise steering (`delivery:"not_delivered"`), and again with a steer answered `promptRequired`: identical outcomes to (h). The council input's worker position reads `"not delivered — …"`.
...
- **Q4. Resolved (operator, 2026-09-24): confirmed.** The ruling's words were "declined by the worker, held by the monitor"; review on #604 (`0493046`) showed that a HIGH the worker never answered, or never received (`delivery:"not_delivered"`), was otherwise auto-approved by a skipped judge via `combine_verdict(true, None)`. The operator confirmed the extension: an unaccepted HIGH — declined, **unanswered, or never delivered** — that the monitor holds is unresolved exactly like a declined-and-held one and takes the same council path (§6.3 item 2, §6.7): council YES → continue autonomously; NO or no verdict → human pause, gate kind `team_dispute`. Rev 13 collapsed every formerly pending clause into the approved text; §6.3 item 2 and acceptance #16 (h)–(i) are approved for build.
