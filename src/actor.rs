...
pub(crate) fn run(
    ...
    for run_id in team_boot_applies {
        ...
        if let Err(e) = team_gate::finish_release_at_boot(&mut cx, &run_id) {
    ...
    loop {
        ...
        match cmd {
            ...
            Command::Launch(spec) => {
                ...
                // outright rather than running it unapproved. `launch_run` carries a plan.
                let res = match crate::plan_gate::launch_plan(
                    &store,
                    plan.as_ref(),
                    workflow.as_deref(),
                    None,
                    ...
                    Ok(_) => Err(anyhow::anyhow!(
                        "a plan or preset launch must use launch_run: its plan_approval gate \
                         cannot be honoured on the straight-through path"
                    )),
                    Err(e) => Err(e),
                };
            ...
            Command::LaunchRun { spec, reply } => {
                ...
                let res: anyhow::Result<String> = (|| {
                    ...
                    // (DES-TEAMING-002 T3) The launch's plan — a user plan, or the steps of the
                    // preset `workflow` names. The score-independent checks run HERE: a refusal
                    // is a synchronous Err (`plan.proposed` + `plan.refused` published) with no
                    // session persisted. Scoring, floor fill and the approval matrix run once the
                    // worktree's base commit is known (`team_plan_at_launch`).
                    let launch_plan = crate::plan_gate::launch_plan(
                        &store,
                        spec.plan.as_ref(),
                    ...
                    if launch_plan.is_none() && spec.deliver_step.is_some() {
                        anyhow::bail!(
                            "a deliver step rides a plan or a preset launch — this launch names \
            ...
            Command::WorktreeReady {
                ...
                // (DES-TEAMING-002 T3) The launch's plan (a user plan or a preset), scored
                // against THIS worktree's base commit, floor-filled, composed into
                // `<run>:plan-1` (a team run) and held for approval when the matrix says so.
                // Its state lands on the stub before `pre_distribute` carries the stub forward.
                ...
                let (team_workflow, team_registry) = match team {
                    Ok(Some((_, id, reg))) => (Ok(Some(id)), Some(reg)),
...
/// (DES-TEAMING-002 §8.4-§8.6, T3) Put a launch's plan — its user-composed `plan`, or the steps
/// of the preset its `workflow` names — through the one plan pipeline: `plan.proposed`, the
/// intent score against the run's graph at `base_commit` (`path.scored`), floor fill, compose,
/// and the approval matrix (`plan.accepted{by:"engine"}`, or held for a `plan_approval` gate that
/// the first step boundary opens). The composed def is `<run>:plan-1` — a TEAM run — registered
...
/// A refused plan is an `Err` (with `plan.refused` published).
...
> {
    ...
    let Some((plan, preset)) = crate::plan_gate::launch_plan(
        &*store,
        spec.plan.as_ref(),
        spec.workflow.as_deref(),
        spec.project_id.as_deref(),
        ...
        if spec.deliver_step.is_some() {
            anyhow::bail!(
                "a deliver step rides a plan or a preset launch — this launch names neither, so \
                 its delivery would be dropped"
            );
        }
        return Ok(None);
    };
    ...
    let decided = crate::plan_gate::decide(
        ...
        crate::plan_gate::Proposal {
            by: "human".into(),
            source: crate::team_events::ProposalSource::Launch {
                session_id: run_id.to_string(),
            },
            kind: crate::team_events::ProposalKind::Initial,
            preset,
            plan,
            reviewing_ord: None,
...
pub(crate) fn launch_run_inner(
    ...
    // (DES-TEAMING-002 T3) A preset (or plan) launch goes through the plan pipeline here too, so a
    // campaign node can never skip its plan_approval gate. This path observes no base commit, so
    // a declared touch set fails closed at 100.
    let repo_root = repo_ref
        .as_deref()
        .and_then(|id| crate::repo::get_repo(store, id).ok().flatten())
        .map(|r| std::path::PathBuf::from(r.root_path));
    ...
    pipeline::plan_and_distribute(
        ...
        team.as_ref().map(|(state, _, _)| state.clone()),
    )?;
    // (T3 round 10) The plan state rode the plan's FIRST session write (`pre_distribute`): no
    // second write, so no crash window where the run is planned but holds no plan state.
...
pub(crate) fn resume_run_inner(
    ...
    // been flipped to `Executing` with no worker behind it.
    //
    // Pre-execution statuses (Planning/Distributing) are skipped because the R1 guard below FAILS
    // them and reaps their worktree — re-provisioning a checkout two blocks before it is reaped is
    // waste, and worse, a re-provision that could not succeed would return Err where R1 owes the
    // caller a deterministic `Failed`. NOT because their workdir is unresolved: `launch_run_inner`
    // calls `resolve_workdir` BEFORE `plan_and_distribute`, so the `Planning` stub that path
    // persists already carries a resolved workdir (src/actor.rs, src/pipeline.rs). The other launch
    // path, `pipeline::run_session`, binds no repo at all and so has no workdir either way.
...
///
/// A workdir that is still a LIVE worktree is left untouched; anything else is re-provisioned.
///
/// The test is [`crate::repo::is_live_worktree`], not `is_dir()` — FINDING-059's lesson, which
/// `create_worktree` already learned, applied one level up. A reap is not always all-or-nothing:
/// `remove_worktree` falls back to `remove_dir_all`, a partial removal leaves the directory SHELL
/// behind, and the `git worktree prune` that follows then deregisters the admin entry *because* the
/// path no longer has a `.git` file. `is_dir()` accepts that shell and returns it as a valid
/// checkout, so the resume would re-dispatch into a directory that is not a boundary at all — the
/// failure FINDING-059 measured, where a worker handed an "empty, unregistered directory" decided to
/// work in the parent repo instead and wrote 297 lines onto the operator's real `master`. A run that
/// resumes into a shell also fails LATER and further from the cause than one that never left here.
///
/// So a non-live workdir goes through `create_worktree`, which is exactly the code that knows how to
/// resolve that state: an empty shell is cleared, pruned, and re-added from the branch; a non-empty
/// one of unknown provenance is refused loudly, here, where the cause is still legible.
fn reprovision_reaped_worktree(
    ...
    if crate::repo::is_live_worktree(std::path::Path::new(&workdir)) {
        return Ok(session);
    }
    ...
    // Name what was actually found: "gone" and "a directory that is no longer a worktree" are
    // different repairs, and an operator reading this line is diagnosing which reap left what.
    let was = if std::path::Path::new(&workdir).exists() {
        "is no longer a live worktree"
    } else {
        "is gone"
    };
    eprintln!(
...
fn advance_or_pause(
    ...
    if let Some(reason) = pause {
        ...
        // its id and publishes no second `gate.opened`.
        ...
        let (reviewing_ord, prompt) = match reason {
            ...
            PauseReason::PlanApproval => {
                ...
                (
                    ...
                    crate::plan_gate::gate_prompt(
                        pending,
                        unit.ord,
                        crate::plan_gate::is_auto(&session.human_confirm),
                    ),
                )
            }
        ...
        let ord = unit.ord;
        // (T3 round 10) The gate's `gate.opened` is QUEUED on the plan state (and the gate made the
        // run's open team gate) in the pause's own batch, then published from the queue once the
        // batch is written: a crash between the two leaves it queued, and the boot publishes it —
        // never a gate id with no `gate.opened` behind it.
        if let Some((gate_id, pending, from_rev)) = &plan_gate_opened {
            if team_gate::publishes(&session) {
                match crate::plan_gate::gate_opened(
                    ...
                    *from_rev,
                    ...
                    Ok(fact) => team_gate::stage_plan_gate_opened(&mut session, gate_id, &fact)?,
                    Err(e) => emit_run_error(subscribers, run_id, e),
                }
            }
        }
        ...
        if reason == PauseReason::PlanApproval {
            ...
            team_gate::flush_queued(store, &mut session)?;
        }
        return Ok(Progress::Paused);
    }
...
fn should_pause(
    ...
    // PLAN APPROVAL (DES-TEAMING-002 §8.6, T3): a plan held for approval pauses the run at the
    // step boundary before ANY further unit — judged first, since every other gate is a gate of
    // a plan nobody has released yet.
    if session
        .team_plan
        .as_ref()
...
/// The layer-3 governance claim at the cursor's phase boundary (crew#32 / DES-EXEC-001 §3):
/// deny-dominates, read by every approval arm of `confirm_gate` BEFORE any approval-side mutation.
...
fn phase_boundary_claim(
    store: &mut dyn GraphStore,
    session: &AgentSession,
    run_id: &str,
) -> anyhow::Result<ConformanceClaim> {
    let units = crate::domain::session_units(store, run_id)?;
...
/// (DES-TEAMING-002 §8.6, T3) The `plan_approval` arm of [`confirm_gate`], entered when the run's
/// OPEN interaction row is a plan gate (DES-001 §6.7: its `gate_kind` is read before the row is
/// resolved). Approve releases the held plan (`gate.decided{human_approved}` →
/// `plan.accepted{by:"human"}`) and dispatches exactly the cursor unit, which must not have run,
/// without bumping the attempt. An edit (`HumanDecision::EditPlan`) is `gate.decided{human_amended}`
/// → `plan.proposed{by:"human", kind:"edit"}` → floor fill → compose, accepted directly as rev n+1
/// (the run re-plans onto `<run>:plan-<n+1>`); a refused edit publishes `plan.refused` and re-opens
/// the gate with a NEW `gate_id`. Reject cancels. Every refusal that can be judged up front is
/// judged before the row is resolved, so a refused answer leaves the gate open.
#[allow(clippy::too_many_arguments)]
fn confirm_plan_gate(
    store: &mut dyn GraphStore,
    subscribers: &mut crate::event_log::EventSink,
    runner: &Arc<dyn StepRunner>,
    self_tx: &Sender<Command>,
    ...
    run_id: &str,
    session: AgentSession,
    row: crate::interaction::InteractionRequest,
    ...
    let state = session
        .team_plan
        .clone()
        .ok_or_else(|| anyhow::anyhow!("run {run_id} is at a plan_approval gate with no plan"))?;
    let pending = state
        .pending
        .clone()
        .ok_or_else(|| anyhow::anyhow!("run {run_id} holds no plan for approval"))?;
    let gate_id = pending
        .gate_id
        .clone()
        .ok_or_else(|| anyhow::anyhow!("run {run_id}'s plan_approval gate has no gate id"))?;
    let units = crate::domain::session_units(store, run_id)?;
    let cursor = units.get(session.unit_ix);
    let ord = row.ord.or(cursor.map(|u| u.ord)).unwrap_or(0);
    let not_run = |u: &crate::domain::WorkUnit| {
        matches!(
            u.status,
            crate::domain::UnitStatus::Pending | crate::domain::UnitStatus::Distributed
        )
    };
    // REFUSE BEFORE RESOLVING (DES-L1 PR-1B M3): the row stays open on every refusal here.
    match &decision {
        HumanDecision::RequestChanges { .. } => anyhow::bail!(
            "a plan_approval gate reviews a plan, not work — approve, approve with an edited \
             plan, or reject"
        ),
        HumanDecision::Approve { amend: Some(a), .. } if !a.is_empty() => anyhow::bail!(
            "a plan_approval gate takes an edited plan, not amend text — send the edited steps"
        ),
        HumanDecision::Approve { .. } | HumanDecision::EditPlan { .. } => {
            // A finished unit is never re-dispatched: the cursor must not have run.
            if !cursor.is_some_and(not_run) {
                anyhow::bail!(
                    "unit {ord} is not pending — a plan_approval gate releases only a unit that \
                     has not run"
                );
            }
    ...
    let answer = match &decision {
        HumanDecision::Approve { .. } => {
            serde_json::json!({"approve": true, "action": "approve", "amend": null})
        }
        HumanDecision::EditPlan { plan } => {
            serde_json::json!({"approve": true, "action": "edit_plan", "amend": null, "plan": plan})
        }
    ...
    // (T3 round 10) The answered rows are NOT written here: they ride the batch that records what
    // the answer does (the staged answer on its pending fact), so no crash can leave the gate
    // answered with nothing staged. `row` is the plan gate's own row — re-opened in memory when
    // its answer was recorded before a crash — resolved with the rest.
    let resolved_at = crate::interaction::now_millis();
    let mut answered = crate::interaction::resolve_open_rows(
        &*store,
        run_id,
    ...
    if !answered.iter().any(|r| r.id == row.id) {
        let mut r = row.clone();
        r.status = crate::interaction::InteractionStatus::Answered;
        r.answer = Some(answer.to_string());
        r.resolved_at = Some(resolved_at);
        answered.push(r);
    }
    let answered: Vec<_> = answered.iter().map(|r| r.to_node()).collect();
    let attempt = session.attempt;
    ...
    let plan = match decision {
        HumanDecision::Reject => {
            crate::domain::put_nodes(store, &answered)?;
    ...
    let claim = phase_boundary_claim(store, &session, run_id)?;
    ...
    team_gate::release_plan_gate(&mut cx, session, decided_ev, release, answered)
}

/// The row of `held`'s gate, re-opened for its answer (not written: the answer's batch writes
/// it resolved). Refused when the gate never opened (its pause was never written) — the run then
/// opens it on its next advance (`resume_run`), never answered blind.
fn reopen_plan_row(
    store: &dyn GraphStore,
    run_id: &str,
...
fn replan_for_accepted_edit(
    ...
    let mut pre = pipeline::pre_distribute(
        ...
        run_id,
        session.human_confirm,
        session.auto_deliver,
        session.repo_ref.clone(),
        session.workdir.clone(),
        session.extra_write_roots.clone(),
        session.extra_read_roots.clone(),
        session.project_graph.clone(),
...
pub(crate) fn confirm_gate(
    ...
    if matches!(decision, crate::workflow::HumanDecision::EditPlan { .. }) {
        anyhow::bail!(
            "run {run_id} has no plan_approval gate open — an edited plan answers only a plan \
             approval gate"
        );
    }
...
mod terminal_worktree_reap_tests {
    ...
    /// Put a failed run on the store with the cursor unit a crew#277 resume re-dispatches, WITHOUT
    /// going through `fail_run`. `fail_run` reaps on a background thread, and these two tests build
    /// a specific on-disk shape afterwards — racing that thread would decide the test's outcome.
    /// Returns the repo root and the worktree path.
    fn failed_run_awaiting_resume(
        store: &mut dyn GraphStore,
        name: &str,
        run_id: &str,
    ) -> (std::path::PathBuf, std::path::PathBuf) {
        use crate::domain::{UnitStatus, WorkUnit};
        let (root, wt) = seeded_with_status(store, name, run_id, SessionStatus::Failed);
        let mut unit = WorkUnit::pending(format!("{run_id}:u1"), run_id, 1, "work");
        unit.assigned_cli = Some("claude".into());
        unit.status = UnitStatus::Rejected;
        unit.denial_reason = Some(format!(
            "{WORKER_FAILURE_MARKER} 1: (cli `claude` exited 1)"
        ));
        put_node(store, unit.to_node()).unwrap();
        (root, wt)
    }

    /// Commit `landed.txt` inside `wt` so `wicked/<run_id>` carries work a re-provision must restore.
    fn commit_landed_work(wt: &Path) {
        std::fs::write(wt.join("landed.txt"), "phase 1 output\n").unwrap();
        for args in [&["add", "-A"][..], &["commit", "-qm", "phase 1"][..]] {
            let out = std::process::Command::new("git")
                .hardened()
    ...
    /// Reduce `wt` to the shape a PARTIAL removal leaves: the directory survives, its `.git` FILE
    /// does not. `remove_worktree` falls back to `remove_dir_all`, which can die part-way (a busy
    /// file, a permissions error, an interrupted process), and the `git worktree prune` that both
    /// reap paths run next then deregisters the admin entry *precisely because* the path lost its
    /// `.git`. `keep` decides whether anything is left inside — the two halves of `create_worktree`'s
    /// recovery rule (an empty shell is repairable; a directory of unknown provenance is not).
    fn break_into_directory_shell(root: &Path, wt: &Path, keep: &[&str]) {
        for entry in std::fs::read_dir(wt).unwrap() {
        ...
        let out = std::process::Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(out.status.success(), "git worktree prune failed");
        // The premise both tests rest on: this still passes `is_dir()`, and is not a worktree.
        assert!(
            wt.is_dir() && !wt.join(".git").exists(),
            "the shell must survive as a directory with no `.git`: {}",
            wt.display()
        );
        assert!(
            !crate::repo::is_live_worktree(wt),
            "a directory with no `.git` file is not a live worktree: {}",
            wt.display()
        );
    }

    /// core#290 / PR #303 review — A DIRECTORY IS NOT A WORKTREE. The re-provision above tested the
    /// recorded workdir with `Path::is_dir()`, so it treated any surviving directory as a usable
    /// checkout and returned early. But a reap is not all-or-nothing: a partial `remove_dir_all`
    /// plus the `worktree prune` that follows leaves a directory SHELL with no `.git` file and no
    /// admin entry — it stats as a directory and is not a worktree. `is_dir()` waves that through,
    /// and the resume then re-dispatches into it: the worker's cwd is not an isolated boundary, the
    /// branch's work is not there, and the run fails somewhere downstream of the actual cause. It is
    /// FINDING-059 one level up — the finding where a worker handed an "empty, unregistered
    /// directory" worked in the parent repo instead and wrote 297 lines onto the operator's `master`.
    ///
    /// Falsified by reverting the check to `if std::path::Path::new(&workdir).is_dir()`:
    ///
    ///   the resume accepted .../wicked-worktrees/r-shell as a workdir, but it is a directory
    ///   shell, not a git worktree — a partial reap left it with no `.git` file, so the worker
    ///   gets a cwd that is not an isolated checkout (core#290 / PR #303)
    #[test]
    fn a_resume_rebuilds_a_workdir_that_survived_as_a_directory_but_not_as_a_worktree() {
        let mut store = open_store(Some(":memory:")).unwrap();
        let (root, wt) = failed_run_awaiting_resume(&mut store, "shell", "r-shell");
        commit_landed_work(&wt);
        // The exact broken shape: an EMPTY shell — the repairable half of the rule.
        break_into_directory_shell(&root, &wt, &[]);
        ...
        let workdir = crate::domain::get_session(&store, "r-shell")
            .unwrap()
            .unwrap()
            .workdir
            .expect("a repo-bound run keeps a workdir across the resume");
        assert!(
            crate::repo::is_live_worktree(Path::new(&workdir)),
            "the resume accepted {workdir} as a workdir, but it is a directory shell, not a git \
             worktree — a partial reap left it with no `.git` file, so the worker gets a cwd that \
             is not an isolated checkout (core#290 / PR #303)"
        );
        assert!(
            Path::new(&workdir).join("landed.txt").is_file(),
            "re-adding the worktree from wicked/r-shell must restore the work the failed run \
             landed; the shell the resume was handed did not carry it"
        );
        let _ = std::fs::remove_dir_all(&root);
    }

    /// The other half of the same rule, and the reason the check belongs HERE rather than downstream:
    /// a shell that still holds files is a directory of unknown provenance, and `create_worktree`
    /// refuses to call it an isolated checkout. The resume must surface that refusal at the cause —
    /// as an `Err` to the caller with the run STILL `Failed` and still resumable, which is what the
    /// re-provision's placement ahead of the cursor reset buys. Under `is_dir()` this run resumed to
    /// `Executing` with a worker pointed at a non-checkout and no error anywhere.
    ///
    /// Falsified by reverting the check to `is_dir()`: the resume returns `Ok(Executing)` and the
    /// `expect` on the error fires.
    #[test]
    fn a_resume_into_a_non_worktree_directory_errors_with_the_run_still_resumable() {
        let mut store = open_store(Some(":memory:")).unwrap();
        let (root, wt) = failed_run_awaiting_resume(&mut store, "shellfull", "r-shellfull");
        commit_landed_work(&wt);
        // Same broken shape, but something survived inside it — unrecoverable, must be refused.
        break_into_directory_shell(&root, &wt, &["landed.txt"]);
        ...
        .expect_err(
            "resuming into a directory that is not a git worktree must fail HERE, where the cause \
             is legible — not hand the worker a non-checkout and fail somewhere downstream",
        );
        let msg = format!("{err:#}");
        assert!(
            msg.contains("is not a git worktree"),
            "the error must name the real cause; got: {msg}"
        );

        let session = crate::domain::get_session(&store, "r-shellfull")
            .unwrap()
            .unwrap();
        assert_eq!(
            session.status,
            SessionStatus::Failed,
            "the re-provision runs BEFORE the cursor reset precisely so a failure leaves the run \
             terminal and resumable, not `Executing` with no worker behind it"
        );
        assert!(
            !in_flight.contains("r-shellfull"),
            "a run that never dispatched must not be marked in-flight — it would refuse the next \
             resume with RunBusy"
        );
        let _ = std::fs::remove_dir_all(&root);
    }
