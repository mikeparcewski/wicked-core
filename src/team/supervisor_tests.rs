...
// ── Review round 2 on #628, D5: a dead attempt's help starts nothing ─────────────────────────────
...
/// and a dead attempt's `help.requested` starts no member turn in any read mode — no tokens are
/// spent answering an attempt nobody waits on.
#[test]
fn t6_d5_no_tail_snapshot_replays_everything_and_a_dead_attempts_help_starts_nothing() {
    ...
    let (cursor, jobs) = replay(&mut b, &h.rig.bus, i64::MAX, None);
    assert!(jobs.is_empty(), "history starts nothing");
    h.core = b;
    h.cursor = cursor;
    h.pump();
