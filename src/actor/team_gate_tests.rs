...
fn a_restart_between_the_ack_and_the_apply_applies_the_edit_exactly_once() {
    ...
    // (round 10, HIGH 2) The boot releases the run through the team gate itself: the accepted
    // rev's `plan.accepted` lands, then the cursor unit dispatches — no resume needed, and never
    // a dispatch ahead of `plan.accepted`.
    wait_for("the first dispatch", || {
        e.runner.0.load(AtomicOrdering::SeqCst) >= 1
    });
    assert!(rig.types("r9b").contains(&tev::PLAN_ACCEPTED.to_string()));
...
/// HIGH 2 (a): a crash after the plan gate's row was answered and before the staged answer was
/// written (the row answered, nothing else) → the next answer is still the PLAN gate's: it is
/// re-opened and answered through the plan arm — `gate.decided`, the accepted rev, then
/// `plan.accepted` before the cursor unit dispatches. Never the generic arm running unit 1 with
/// no accepted rev.
#[test]
fn an_answered_plan_row_without_its_staged_answer_is_answered_through_the_plan_arm() {
    let rig = rig("r10ans");
    let e = engine(&rig, fast(&rig));
...
/// HIGH 2 (b): a reassign while the run is Executing with its plan still held (its
...
/// HIGH 2 (b): the launch persisted `Executing` with the plan held and crashed before its pause
/// was written → after a restart a reassign dispatches nothing, and a resume stops at the plan
/// gate.
#[test]
fn a_run_left_executing_with_its_plan_held_dispatches_nothing_after_a_restart() {
    let rig = rig("r10ex");
    let e = engine(&rig, fast(&rig));
    at_the_plan_gate(&rig, &e, "r10e");
    let db = e.db.clone();
    drop(e);
    let mut store = wicked_apps_core::open_store_any(Some(&db)).expect("store opens");
    let mut session = crate::domain::get_session(&store, "r10e").unwrap().unwrap();
    session.status = SessionStatus::Executing;
    if let Some(p) = session.team_plan.as_mut().and_then(|t| t.pending.as_mut()) {
        p.gate_id = None;
    }
    if let Some(t) = session.team.as_mut() {
        t.open_gate = None;
    }
    crate::domain::put_node(&mut store, session.to_node()).unwrap();
    crate::interaction::resolve_open_for_session(
    ...
    std::thread::sleep(Duration::from_millis(300));
    let e = engine_on(&db, fast(&rig));
    assert!(e.core.reassign_unit("r10e", 1, Some("b".into())).is_err());
    std::thread::sleep(Duration::from_millis(300));
    assert_eq!(runs(&e), 0);
    let _ = e.core.resume_run("r10e");
    wait_for("the plan_approval gate", || {
        open_gate_kinds(&db, "r10e") == plan_gate_kind()
    });
    assert_eq!(runs(&e), 0);
}

/// MEDIUM 4: a campaign node naming a preset crashes right after it is planned and distributed →
/// after a restart the run still holds its plan: it stops at `plan_approval`, never runs a unit
/// unapproved on a unit-derived `plan.accepted`.
#[test]
fn a_crash_right_after_a_campaign_node_is_planned_keeps_its_plan_gate() {
    let rig = rig("r10camp");
    let e = engine(&rig, fast(&rig));
    let run = "camp10:n1:a0";
    crate::failpoint::arm("launch.after_plan", run);
    let _ = e.core.launch_campaign(crate::CampaignDef {
        id: "camp10".into(),
        ...
        name: "camp10".into(),
        edges: vec![],
        policy: crate::campaign::FailurePolicy::FailFast,
        max_concurrency: 1,
        denial_gate: Default::default(),
    });
    let db = e.db.clone();
    wait_for("the planned run", || {
        let store = wicked_apps_core::open_store_any(Some(&db)).expect("store opens");
        crate::domain::get_session(&store, run)
            .ok()
            .flatten()
            .is_some_and(|s| s.status == SessionStatus::Executing)
    });
    drop(e);
    std::thread::sleep(Duration::from_millis(300));
    let e = engine_on(&db, fast(&rig));
    let _ = e.core.resume_run(run);
    wait_for("the plan_approval gate", || {
        open_gate_kinds(&db, run) == plan_gate_kind()
    });
    assert_eq!(runs(&e), 0);
}
...
/// MEDIUM 6: a crash right after the plan gate's pause is written (its gate id minted) and
/// before its `gate.opened` is published → after a restart `gate.opened` is on the bus, once.
#[test]
fn a_crash_after_the_plan_gate_pause_still_publishes_its_gate_opened_once() {
    let rig = rig("r10op");
    let e = engine(&rig, fast(&rig));
    crate::failpoint::arm("plan_gate.after_pause_batch", "r10o");
    let _ = try_launch_plan(
        &e,
        "r10o",
        serde_json::json!({"steps": [{"catalog": "build", "id": "build"}]}),
        HumanConfirm::None,
    ...
    let db = e.db.clone();
    wait_for("the plan gate row", || {
        open_gate_kinds(&db, "r10o") == plan_gate_kind()
    });
    std::thread::sleep(Duration::from_millis(300));
    assert!(!rig.types("r10o").contains(&tev::GATE_OPENED.to_string()));
    drop(e);
    std::thread::sleep(Duration::from_millis(300));
    let e = engine_on(&db, fast(&rig));
    wait_for("gate.opened", || {
        count(&rig.types("r10o"), tev::GATE_OPENED) == 1
    });
    std::thread::sleep(Duration::from_millis(500));
    assert_eq!(count(&rig.types("r10o"), tev::GATE_OPENED), 1);
    drop(e);
}
