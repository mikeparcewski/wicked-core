...
//!   wicked-core rules retire (--id <RULE-ID>)... [--doc <PATH>]... --manifest <M.json> \
//!       [--out <receipt.json>]   # OPERATOR-ONLY kill switch (AW-24): retire the named rules
//!       # across EVERY lane their fan-out manifest rows name, in ONE op — enforcement copy
//!       # retired, every discovery-graph copy retired, knowledge rationale marked non-normative —
//!       # each cli lane re-opened fresh and verified gone from recall. --doc retires every rule
//!       # the manifest derived from that doc path (the deleted-doc → explicit-retire bridge:
//!       # `rules drift` reports the orphans, this clears them). A daemon-held enforcement store
//!       # is NEVER CLI-written: the receipt records the pending crew-api DELETE instead.
...
const SUBCOMMAND_USAGE: &[(&str, &str)] = &[
    ...
    (
        ...
        "wicked-core rules ingest <dir> [--db <F>]\n  \
         ...
         wicked-core rules retire (--id <RULE-ID>)... [--doc <PATH>]... --manifest <M.json> \
         [--out <receipt.json>]\n  \
         OPERATOR-ONLY kill switch (AW-24 / arch-R22): retire the named rules across EVERY lane \
         their fan-out manifest rows name, in ONE op — enforcement copy retired, every discovery \
         copy retired, knowledge rationale marked non-normative — each cli lane re-opened FRESH \
         and verified gone from recall; the receipt is the audit record. --doc selects every rule \
         the manifest derived from that doc path (deleted doc → explicit retire, never silent \
         orphaning). Retire-not-delete: nodes survive so past decisions stay explicable. A \
         daemon-held enforcement store is NEVER CLI-written — the receipt records the pending \
         crew-api DELETE + rules/preview verification instead. WRITES to every cli lane store.\n\
