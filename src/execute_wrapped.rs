...
/// the argv isolation, the ACP config-dir override, and the skills-snapshot handoff
/// (`skills_snapshot::admit_unit`) cannot disagree about it: a worker that inherits the operator's
/// configuration inherits their plugins too, and must be handed no snapshot.
...
pub(crate) fn inject_isolation_flags(
    ...
    if !argv_states(&stated, &["--disallowedTools", "--disallowed-tools"]) {
        // CARVED around the snapshot (`deny_rules_for`, core#396): the default snapshot location
        // is under `~/.wicked-crew`, which this fence denies, and Claude's deny beats any allow —
        // so the deny itself must not cover the snapshot.
        let rules = deny_rules_for(skills_plugin);
        if !rules.is_empty() {
    ...
    // from the argv ALWAYS, snapshot in hand or not, and said so. Two `--plugin-dir`s naming the
    // same plugin load it twice and the stale one is the very defect; and WITHOUT a snapshot the
    // template's flag is not "the worker's only source of skills" — it is the stale hand copy the
    // ladder exists to keep out. The ladder decides what a worker gets: a unit that needs a skill
    // when the ladder yields no root is refused before launch, never handed the hand copy. No
    // deference here, unlike the flags above: the template's choice was a workaround for the gap
    // this closes.
...
/// lives.
///
/// # The carve-out (core#396)
///
/// `skills_root` is the skills snapshot handed to this launch, or `None` for the blanket fence.
/// The Read fence is CARVED around the snapshot: the default snapshot location is
/// `<crewStateHome>/skills/snapshots/<gen>` — under
/// `~/.wicked-crew`, which this fence denies for every file tool — and Claude's deny beats any
/// allow, so no allow rule can open it: the deny pattern itself must not cover the snapshot. For
/// each denied directory that is an ancestor of the snapshot, the single `Read(<dir>/**)` is
/// replaced by one Read rule per SIBLING entry on the path down to the snapshot (`core.db`,
/// `daemon-stdout.log`, `skills/effective/**`, `skills/snapshots/6/**`, `skills/current/**`, …),
/// enumerated from the live tree — so every other entry stays denied while no Read rule names the
/// snapshot. `Edit`/`Write` keep the blanket `<dir>/**`: the snapshot is immutable by contract and
/// a worker never writes under it. Both carriers use this (the `--disallowedTools` argv and the
/// `permissions.deny` of the wrapped and ACP settings files), so the fence cannot differ by path.
///
/// Stated limits: the enumeration is a point-in-time listing, so a sibling CREATED after launch is
/// fenced by Edit/Write only until the next launch (the daemon's state home is populated at
/// start-up; the carve is logged so an operator can see what it enumerated). No wildcard
/// (`<dir>/*`) is used for the siblings on purpose: a matcher that read it as covering the
/// directory's contents would deny the snapshot silently, which is the exact failure this
/// function exists to prevent — and a boundary you cannot verify is not one.
pub(crate) fn deny_rules_for(skills_root: Option<&Path>) -> Vec<String> {
    ...
    for dir in dirs {
        ...
        match skills_root.and_then(|root| fence_base(&dir, root).map(|base| (base, root))) {
            Some((base, root)) => {
                rules.extend(carved_read_rules(&dir, &base, root));
...
/// The spelling of the fenced `dir` that `root` (a snapshot's canonical real path) sits under —
/// `dir` as derived from `$HOME`, else its canonical form when the home itself is a symlink — or
/// `None` when the fence does not cover the snapshot at all (nothing to carve).
fn fence_base(dir: &Path, root: &Path) -> Option<PathBuf> {
...
/// The Read rules for `fence` with `root` carved out — see [`deny_rules_for`]. `base` is the
/// spelling of `fence` that `root` is under. Walks from `base` down to `root`'s parent: at every
/// level, every entry that is not the next step towards the snapshot is denied — as itself and, if
/// it is not a plain file (a directory, or a symlink such as crew's `current`), as a tree. Sorted
/// per level so the rule list is deterministic whatever order the directory iterates in.
fn carved_read_rules(fence: &Path, base: &Path, root: &Path) -> Vec<String> {
    let mut rules = Vec::new();
    let Ok(rel) = root.strip_prefix(base) else {
        return rules;
    };
    let mut level = base.to_path_buf();
    let mut fenced = 0usize;
    for component in rel.components() {
        let next = level.join(component);
        match std::fs::read_dir(&level) {
            Ok(entries) => {
                ...
                for sibling in siblings {
                    let Some(sp) = rule_path(&sibling) else {
                        eprintln!(
                            "wicked-core: worker isolation cannot express a deny rule for {} \
                             (non-UTF8, or it contains a backslash or a comma); this path is NOT \
                             fenced off from workers",
                            sibling.display()
                        );
                        continue;
                    };
                    rules.push(format!("Read({sp})"));
                    if !std::fs::symlink_metadata(&sibling).is_ok_and(|m| m.is_file()) {
                        rules.push(format!("Read({sp}/**)"));
                    }
                    fenced += 1;
                }
            ...
            Err(e) => eprintln!(
                "wicked-core: worker isolation cannot list {} to carve the Read fence around the \
                 skills snapshot ({e}); the entries at that level are NOT fenced off from Read",
                level.display()
            ),
        }
        level = next;
    ...
    eprintln!(
        "[wicked-core] skills.notice the Read fence for {} is carved around the skills snapshot at \
         {}: {fenced} sibling entries denied individually; Edit/Write stay denied for the whole \
         tree, and a sibling created after this launch is fenced by Edit/Write only until the \
         next launch",
        fence.display(),
        root.display()
    );
...
impl WrappedCliStepRunner {
    ...
    fn exec(&self, input: &StepInput, emit: &DeltaSink) -> StepOutput {
        ...
        let is_claude = tokenize(&invocation)
            .first()
            .is_some_and(|b| binary_is_claude(b));
        let worker_cli = crate::skills_snapshot::WorkerCli::for_seat(is_claude, &cli_key);
        ...
        let form = SkillForm::for_claude(is_claude);
        ...
        if is_claude {
            ...
            // evaluator ran under codex's default read-only sandbox and refused every write/temp/
            // socket, stranding otherwise-correct work at the adversarial-review gate. The built-in
            // codex posture is BOUNDED `--sandbox workspace-write` (registry.rs): writes confined to
            // the worktree, denied outside — codex's own sandbox is the boundary, which is safe
        ...
        let (status, output, usage, files, tools) = if argv.is_empty() {
            ...
            // `$TMPDIR` writes / vitest's `.vite-temp` + sockets landed in the system temp — OUTSIDE
            // codex's workspace-write root, where they are DENIED ("managed read-only sandbox").
            // Lifted here it runs for claude and non-claude alike, governed or not: it is a scratch
            ...
            match bounded {
                ...
                Ok((0, out, _, usage, files, tools)) => {
                    // pi banner strip at capture (core#268; ONE line — audit windows below).
                    let out = crate::acp_runner::strip_pi_banner_owned(out);
                    ...
                    // FINDING-101's deliverable floor USED to live here, and that was the bug
                    // (core#297 §1): a check that only the WRAPPED runner performs makes "did this
                    // phase produce what it promised" depend on which seat the run resolved to —
                    // the ACP path (a real `claude` seat; every governed dogfood run) parsed
                    // `required_deliverables` and never read it. It now runs once, at the
                    // runner-independent fold in `actor::apply_step_result`, where the unit and the
                    // session's workdir + declared write roots are all in hand. Do NOT re-add it
...
/// escape.
fn armed_write_roots(
    cwd: &Path,
    extras: &[String],
    graph_dir: Option<&Path>,
) -> std::ffi::OsString {
    // No widening ⇒ the pre-#259 boundary, byte-identical — and no join_paths round-trip that
...
/// `mktemp`, `> $TMPDIR/x`, vitest's `.vite-temp`/sockets and every tool honoring the platform temp
/// env then land in-boundary — reaped with the sandbox/worktree, and inside codex's own
/// `workspace-write` root — instead of the system temp (outside it, where a bounded sandbox denies
/// them). Applied for EVERY seat, unlike the boundary-arming that stays claude-only: this is a
/// scratch convenience, not a governance control. Set ONLY when the dir really exists as a
/// directory — a temp env pointing at nothing (mkdir refused: a pre-existing `tmp` FILE, a read-only
/// FS) breaks every tool that consults it; the un-set fallback is the system temp.
pub(crate) fn redirect_scratch_into_boundary(cmd: &mut Command, cwd: &Path) {
    let unit_tmp = cwd.join("tmp");
    if std::fs::create_dir_all(&unit_tmp).is_ok() && unit_tmp.is_dir() {
...
pub(crate) fn sandbox_for(input: &StepInput) -> PathBuf {
    sandbox_root(&input.run_id)
}

/// [`sandbox_for`] keyed by run id alone — the spelling the ACTOR needs. The deliverable floor
/// (core#297) runs at the runner-independent fold, which holds a session rather than a
/// [`StepInput`]; deriving the sandbox path there from a COPY of this join would be the classic
/// two-definitions-one-truth bug, where a change here silently stops the floor from finding the
/// files an unbound run actually wrote.
pub(crate) fn sandbox_root(run_id: &str) -> PathBuf {
    std::env::temp_dir()
...
struct GovLaunch {
    ...
    /// LAUNCHER-declared extra READ-ONLY roots (core#294), copied from
    /// [`crate::workflow::GovernanceContext::extra_read_roots`] — already validated at launch.
...
///     `code_graph_db` can still reach here from an old record. Pushed as a READ root, the gate's
...
/// subprocess (`WICKED_READ_ROOTS`); the ACP path hands them to the in-process boundary. Two
...
fn arm_input_governance(
    ...
    let settings = serde_json::json!({
        ...
        "permissions": { "deny": deny_rules_for(skills_root), "allow": ["mcp__wicked-estate"] }
    });
...
/// overlaid with the operator's `clis.toml`, the operator's override winning) — resolved the same
/// way, from the same source, as the council vote path (`wicked_council::dispatch`, which appends
/// `cli.trust_flags` to a voting seat's argv). Empty when the seat is unknown or declares none.
///
/// crew#427: the governed-worker path (`exec`) used to drop this entirely — `assigned_invocation`
/// and [`resolve_invocation`] carry only the `headless_invocation` template, never `trust_flags` —
/// so a non-claude evaluator (codex) ran under codex's DEFAULT read-only sandbox and refused every
...
/// path (`wicked_council::dispatch` `argv.extend(cli.trust_flags)`), but placed BEFORE any `--`
/// guard that [`build_argv`] inserted: the guard makes everything after it positional, so appending
/// a flag past it would be read as a prompt argument, not an option. When there is no guard (the
/// codex template — the prompt follows a bare flag, so `build_argv` adds none), this appends at the
/// end, byte-identical to the vote path. Claude is never routed here: its boundary is the gate-hook
/// plus the isolation flags, and applying `--dangerously-skip-permissions` would make those deny
/// rules inert (see [`inject_isolation_flags`]).
pub(crate) fn apply_seat_posture(argv: &mut Vec<String>, posture: &[String]) {
    // An empty argv is a no-invocation unit that fails hard below; never turn it into a runnable
    // one by making a posture flag argv[0].
    if posture.is_empty() || argv.is_empty() {
        return;
    }
    match argv.iter().position(|a| a == "--") {
        Some(guard) => {
            for (k, flag) in posture.iter().enumerate() {
                argv.insert(guard + k, flag.clone());
            }
        }
...
pub(crate) enum SkillForm {
    ...
    ClaudePlugin,
    /// The bare mirrored directory name (the frontmatter `name`).
    MirroredName,
}

impl SkillForm {
    /// The form for a seat already known to be (or not be) claude — the wrapped and PTY runners
    /// decide that off the template's binary, the ACP runner off the seat's registry record.
    pub(crate) fn for_claude(is_claude: bool) -> Self {
        if is_claude {
    ...
    /// token) — the same fact the wrapped runner selects its output adapter and isolation by.
...
mod tests {
    ...
    fn estate_home_grants_are_per_key_precise_and_siblings_stay_out_of_reach() {
        ...
        assert_ne!(
            read_a, estate_root,
            "NEVER the repo-graphs root — that is every repo's graph at once"
        );

        // (b) sibling precision: key B's dir is not under the grant computed for key A.
        assert!(
            !key_dir_b.starts_with(&read_a) && !key_dir_b.starts_with(&write_a),
        ...
        assert_eq!(
            repo_read_root_at(db_a.to_str(), Some(&base.join("other-root"))),
            None,
            "a key dir under a DIFFERENT root is not a graph — no grant"
        );
        assert_eq!(
            repo_read_root_at(
                estate_root
                    .join(crate::code_graph::CODE_GRAPH_DB_FILE)
                    .to_str(),
                Some(&estate_root)
            ),
            None,
            "a db directly under the repo-graphs root has no key dir to grant"
        );
        assert_eq!(
            graph_write_dir_at(rel.to_str(), Some(&estate_root)),
            None,
            "the write grant fails closed on the same shapes"
        );
    }

    /// core#294 — the LAUNCH-declared extra READ roots ride the ONE shared assembly, appended to
    /// the evidence-derived set without disturbing it; declaring none reproduces the pre-#294
    /// assembly byte for byte. (That the assembly feeds ONLY read lists — env `WICKED_READ_ROOTS`
    /// and the ACP `BoundaryCtx.roots.read` — is pinned by
    /// `gate_hook::boundary_tests::the_launcher_arms_the_launch_declared_read_roots` and by the
    ...
    fn launch_declared_extra_read_roots_ride_the_shared_assembly() {
        ...
        assert_eq!(
            &widened[..base.len()],
            &base[..],
            "the evidence-derived set must be untouched by the widening"
        );
        assert_eq!(
            widened.len(),
            base.len() + 1,
            "exactly the declared root is added — nothing implicit rides along"
        );
        assert_eq!(
            widened.last().map(std::path::PathBuf::as_path),
            Some(Path::new(&declared)),
            "the declared root itself is what lands in the read list"
        );
    }
    ...
    /// evaluator≠creator router moves review/test units onto, and its posture must be codex's native
    /// `--sandbox workspace-write` — writes confined to the worktree, denied outside — NOT the
    /// unbounded `--dangerously-bypass-approvals-and-sandbox`: with codex's own sandbox off AND
    /// wicked-core's boundary gate being claude-only, a codex worker would be free to write outside
    /// its worktree. Pinned against BUILT-INS (no user path) so it holds regardless of the operator's
    /// live clis.toml, and asserts the EXACT flags land on a codex-shaped argv.
    #[test]
    fn codex_builtin_posture_is_bounded_workspace_write_not_full_bypass() {
        let posture = seat_posture_from("codex", None);
        assert_eq!(
            posture,
            vec!["--sandbox".to_string(), "workspace-write".to_string()],
            "the built-in codex seat must declare the BOUNDED workspace-write posture"
        );
        assert!(
            !posture.iter().any(|f| f.contains("dangerously-bypass")),
            "the built-in codex posture must NOT be the unbounded full-bypass (crew#427)"
        );
        // The codex template's prompt follows a bare flag, so `build_argv` adds no `--` guard and the
        // posture appends at the tail, byte-identical to the council vote path.
        let mut argv = build_argv(
        ...
        assert!(
            !argv.iter().any(|a| a == "--"),
            "precondition: the codex template yields no `--` guard"
        );
        apply_seat_posture(&mut argv, &posture);
        assert_eq!(
            &argv[argv.len() - 2..],
            &["--sandbox".to_string(), "workspace-write".to_string()],
            "the exact posture flags must appear, in order, on the argv"
        );
    }
    ...
    #[test]
    fn apply_seat_posture_inserts_before_the_prompt_guard() {
        let mut guarded = vec![
            "/bin/sh".to_string(),
            "probe".to_string(),
            "--".to_string(),
        ...
        assert_eq!(
            ...
            "posture flags go before the `--` guard, in order"
        );

        let mut empty: Vec<String> = vec![];
        apply_seat_posture(&mut empty, &["--x".to_string()]);
        assert!(
            empty.is_empty(),
            "posture must never make a flag the argv[0] of an empty (no-invocation) argv"
        );

        let mut no_posture = vec!["codex".to_string()];
        apply_seat_posture(&mut no_posture, &[]);
        assert_eq!(no_posture, vec!["codex"], "empty posture is a no-op");
    }

    /// crew#427, part 1 (the wiring). The governed-worker path used to DROP a non-claude seat's
    /// declared posture — `assigned_invocation`/`resolve_invocation` carry only the headless template
    /// — so a codex evaluator ran under codex's default READ-ONLY sandbox and refused every
    /// write/temp/socket. Prove the runner now extends a codex-key seat's real argv with its declared
    /// posture, END-TO-END, and that the posture is the BOUNDED form (not the full-bypass). HOME is
    /// pinned at a clis.toml-free dir so the runner resolves the SHIPPED built-in deterministically.
    #[cfg(unix)]
    #[test]
    fn governed_worker_argv_for_a_codex_seat_carries_its_bounded_posture() {
        ...
        let mut u = WorkUnit::pending("s:u1", "s", 1, "do it");
        // The evaluator≠creator router assigns the review unit to codex, carrying only the headless
        // template as `assigned_invocation` — exactly the shape that dropped the posture before.
        u.assigned_cli = Some("codex".to_string());
        u.assigned_invocation = Some(format!("/bin/sh {} {{PROMPT}}", probe.display()));
        let input = StepInput {
            run_id: "run-posture".to_string(),
            unit_ix: 0,
            attempt: 0,
            unit: u,
            workflow_id: "wf-x".to_string(),
        ...
        assert!(
            ...
            "the governed-worker argv for a codex-key seat must carry its `--sandbox workspace-write` \
             posture (crew#427: it was dropped); got: {}",
            out.output
        );
        assert!(
    ...
    /// crew#427, part 3 (in-boundary scratch for EVERY seat). The `TMPDIR`/`TMP`/`TEMP` → `<cwd>/tmp`
    /// redirect used to live inside the claude-only `gov_env` arm, so a NON-claude seat got none — its
    /// `mktemp`/`$TMPDIR` writes (vitest's `.vite-temp` + sockets) landed in the system temp, OUTSIDE
    /// codex's `workspace-write` root, where they are denied. Lifted out, it now applies to a
    /// non-claude seat too: the probe (a non-claude seat) sees `TMPDIR` pointed at its own `<cwd>/tmp`.
    #[cfg(unix)]
    #[test]
    fn scratch_redirect_applies_to_a_nonclaude_seat() {
        ...
        std::fs::create_dir_all(&dir).unwrap();
        let probe = dir.join("probe.sh");
        std::fs::write(&probe, "echo \"TMP=[${TMPDIR:-UNSET}]\"\n").unwrap();

        let mut u = WorkUnit::pending("s:u1", "s", 1, "do it");
        // A NON-claude seat (binary is /bin/sh, not claude) with NO governance — the exact case the
        // old claude-only placement skipped.
        u.assigned_cli = Some("codex".to_string());
        u.assigned_invocation = Some(format!("/bin/sh {} {{PROMPT}}", probe.display()));
        let input = StepInput {
        ...
        let expected = dir.join("tmp");
        assert!(
            out.output.contains(&expected.display().to_string()),
            "a non-claude seat must get its scratch redirected to <cwd>/tmp ({}); got: {}",
            expected.display(),
            out.output
        );
        assert!(
            !out.output.contains("TMP=[UNSET]"),
            "TMPDIR must be set for a non-claude seat, not left to the system temp; got: {}",
            out.output
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn armed_write_roots_is_cwd_first_plus_declared_extras() {
        ...
        let joined = armed_write_roots(cwd, std::slice::from_ref(&inbox), None);
        ...
        assert_eq!(
            ...
            "cwd first, then extras, then the graph's own key dir"
        );
        ...
        assert_eq!(
            armed_write_roots(cwd, &[bad], None),
            cwd.as_os_str(),
    ...
    fn the_worker_mcp_never_receives_the_operational_store() {
        ...
        let graph_db = Path::new("/abs/repo")
            .join(crate::code_graph::code_graph_rel())
            .to_string_lossy()
            .into_owned();
    ...
    /// so a skill whose directory diverged from its frontmatter name still resolves on both sides,
    /// and a NESTED skill — which Claude Code's plugin loader does not expose — gets no invented
    /// plugin form; without a snapshot, the convention garden's top-level layout guarantees is
    /// applied for Claude and the ref passes through untouched for the rest.
    #[test]
    fn the_skill_directive_is_cli_aware() {
        ...
        assert_eq!(
            plugin_skill_invocation("wicked-garden-test-oracle", claude, Some(&snap)),
            "wicked-garden:qe-oracle",
            "the index wins over the naming convention when the dir diverged from the name"
        );
        assert_eq!(
            plugin_skill_invocation("wicked-garden-qe-oracle", mirrored, Some(&snap)),
            "wicked-garden-test-oracle",
            "a dir-derived ref is spelled by its frontmatter name for the mirrors"
        );
        assert_eq!(
    ...
    /// RAII restore of one process-global variable — the skills-snapshot input — with the same
    /// discipline as [`HomeGuard`]: hold [`ENV_LOCK`] and declare the guard after the lock guard.
    #[cfg(unix)]
    struct VarGuard {
        key: &'static str,
    ...
    /// core#396 × the Read deny fence (FINDING-067): the default snapshot location is
    /// `<crewStateHome>/skills/snapshots/<gen>` — under `~/.wicked-crew`, which the fence denies for
    /// every file tool, and Claude's deny beats allow. The fence is CARVED around the snapshot:
    /// judged with the rule matcher's semantics, a support-file Read under the snapshot is denied by
    /// NO rule, while every worker-state path beside it — the store, the daemon log, the editable
    /// root, the previous generation, the `current` link — is still denied, and Edit/Write under the
    /// snapshot stay denied. Without a snapshot the fence is the blanket it always was, and a
    /// snapshot outside every fenced directory carves nothing. Both carriers call the same function.
    #[cfg(unix)]
    #[test]
    fn the_read_fence_is_carved_around_the_snapshot_and_still_denies_the_worker_state_beside_it() {
        use crate::skills_snapshot::test_support::{scratch, snapshot_root};
        let _guard = ENV_LOCK.write().unwrap_or_else(|p| p.into_inner());
        let home = scratch("carve-home");
        let _home = HomeGuard::pin(&home);
        ...
        let gen6 = snapshot_root(
            &skills.join("snapshots").join("6"),
            "6",
            &[("domain", "wicked-garden-domain")],
        );
        let gen7 = snapshot_root(
            &skills.join("snapshots").join("7"),
            "7",
            &[("domain", "wicked-garden-domain")],
        );
        std::fs::create_dir_all(gen7.join("scripts")).unwrap();
        std::fs::write(gen7.join("scripts").join("extract.py"), b"print()").unwrap();
        std::os::unix::fs::symlink("snapshots/7", skills.join("current")).unwrap();

        let carved = deny_rules_for(Some(&gen7));
        let denying = |tool: &str, path: &Path| -> Vec<&str> {
            carved
                .iter()
                .map(String::as_str)
        ...
        for p in [
            ...
            assert!(
                denying("Read", &p).is_empty(),
                "{} must be readable under the carved fence, denied by {:?}",
                p.display(),
                denying("Read", &p)
            );
            assert!(
                !denying("Edit", &p).is_empty() && !denying("Write", &p).is_empty(),
                "writes under the snapshot stay denied: {}",
                p.display()
            );
        }
        // Everything beside it on the way down: still denied for Read.
        for p in [
            crew.join("core.db"),
            crew.join("daemon-stdout.log"),
            skills.join("manifest.json"),
            effective.join("SKILL.md"),
            gen6.join("skills").join("domain").join("SKILL.md"),
            skills
                .join("current")
                .join("skills")
                .join("domain")
                .join("SKILL.md"),
            skills.join("baseline").join("anything"),
        ] {
            assert!(
                !denying("Read", &p).is_empty(),
                "{} must stay denied for Read: {carved:?}",
                p.display()
            );
        }
        // The crew home's blanket rule is gone from Read ONLY; every other fenced home is untouched.
        let crew_rule = rule_path(&crew).expect("expressible");
        assert!(
            !carved.contains(&format!("Read({crew_rule}/**)")),
            "{carved:?}"
        );
        assert!(
            carved.contains(&format!("Edit({crew_rule}/**)"))
                && carved.contains(&format!("Write({crew_rule}/**)")),
            "{carved:?}"
        );
        let claude_rule = rule_path(&home.join(".claude")).expect("expressible");
        assert!(
            carved.contains(&format!("Read({claude_rule}/**)")),
            "{carved:?}"
        );
        for r in &carved {
            assert!(!r.contains(','), "rule would split when joined: {r}");
        }
        // No snapshot ⇒ the blanket fence.
        let blanket = deny_rules_for(None);
        assert!(
            blanket.contains(&format!("Read({crew_rule}/**)")),
            "{blanket:?}"
        );
        // A snapshot outside every fenced directory carves nothing.
        let elsewhere = snapshot_root(&home.join("elsewhere").join("7"), "7", &[]);
        assert_eq!(deny_rules_for(Some(&elsewhere)), blanket);
        let _ = std::fs::remove_dir_all(&home);
    }
    ...
    /// to a different directory in each generation), and neither tree is written into — the
    /// snapshot is a shared-nothing input, so reaping one generation cannot touch a session on
    /// the other. The ACP runner's session-bound variant lives in `acp_runner::tests`.
    #[test]
    fn two_sessions_on_two_generations_carry_their_own_roots_concurrently_and_write_nothing() {
        ...
        let root1 = snapshot_root(
            &base.join("snapshots").join("1"),
            "1",
            &[("mem", "wicked-garden-mem")],
        );
        let root2 = snapshot_root(
            &base.join("snapshots").join("2"),
            "2",
            &[
        ...
        assert!(
            b.1.contains("\"wicked-garden:mem-v2\""),
            "gen 2 spells the same skill by ITS directory: {}",
            b.1
        );
    ...
    /// END TO END through `run_unit`, the positive half and the negative exclusion: a claude unit
    /// is launched with the snapshot as its ONE `--plugin-dir`, the stale hand copy the operator's
    /// template pinned is stripped from the real argv, the directive names the skill in Claude's
    /// plugin form BY DISCOVERY — the skill lives at `skills/domain-v5/` and is found through the
    /// index by its frontmatter name, so the directive says `wicked-garden:domain-v5`, which no
    /// string rewrite of the ref could produce — and the snapshot tree is byte-identical
    /// afterwards. HOME is pinned so the deny rules and the ladder resolve deterministically.
    #[cfg(unix)]
    #[test]
    fn a_wrapped_claude_unit_is_handed_the_snapshot_and_not_the_stale_hand_copy() {
        ...
        let snapshot = snapshot_root(
            &home.join("snapshots").join("5"),
            "5",
            &[
                ("domain-v5", "wicked-garden-domain"),
                ("mem", "wicked-garden-mem"),
            ],
        );
        let _snap = VarGuard::set(crate::skills_snapshot::SKILLS_SNAPSHOT_ENV, &snapshot);
        let before = tree_fingerprint(&snapshot);
        // The operator's stop-gap: a hand copy pinned by the template's own --plugin-dir.
        ...
        u.skill_ref = Some("wicked-garden-domain".to_string());
        u.assigned_invocation = Some(format!(
            "{} --plugin-dir {} -p {{PROMPT}}",
            claude.display(),
            stale.display()
        ));
        ...
        let argv: Vec<String> = std::fs::read_to_string(&argv_file)
            .expect("the fake claude ran and recorded its argv")
            .lines()
            .map(str::to_string)
            .collect();
        assert_eq!(
        ...
        assert!(
            ...
            "the template's stale hand copy must not reach the worker: {argv:?}"
        );
        assert!(
            argv.iter().any(|a| a.starts_with(
                "Invoke your skill \"wicked-garden:domain-v5\" (via the Skill tool) and complete this task under its instructions: extract the rules"
            )),
    ...
    /// never a silent fallback, never a worker told to invoke a skill it cannot have.
    #[cfg(unix)]
    #[test]
