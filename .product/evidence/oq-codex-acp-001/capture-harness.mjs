#!/usr/bin/env node
// OQ-CODEX-ACP-001 evidence capture harness.
// Minimal ACP JSON-RPC-2.0-over-NDJSON client speaking directly to the codex-acp
...
// Modeled on .product/evidence/oq-pi-acp-001/capture-harness.mjs.
...
const CODEX_ACP_BIN = process.argv[2]
const FIXTURE_CWD = process.argv[3]
const CAPTURE_FILE = process.argv[4]
const SCENARIO = process.argv[5] || 'allow'

if (!CODEX_ACP_BIN || !FIXTURE_CWD || !CAPTURE_FILE) {
  console.error('usage: capture-harness.mjs <codex-acp-bin> <fixture-cwd> <capture-file> <allow|reject>')
  process.exit(2)
}
...
const child = spawn(CODEX_ACP_BIN, [], {
  cwd: FIXTURE_CWD,
...
function handleIncomingRequest(msg) {
  ...
  if (method === 'fs/read_text_file') {
    // The harness advertises fs:{} (no fs methods) to byte-mirror the real client; an adapter
    // sending this anyway is itself capability-noncompliant. Record that loudly rather than
    // silently serving a method we declared absent.
    record('harness-meta', JSON.stringify({ note: 'adapter sent fs/read_text_file despite fs:{} capabilities — capability-noncompliant; served for capture continuity' }));
    try {
...
async function main() {
  await request('initialize', {
    ...
    // client ANSWERS session/request_permission, so a codex-acp that gated permission
    // requests on that capability would now be forced to send them.
  ...
  const promptText = [
    ...
    `3. Run the shell command: echo hello-from-codex`,
