...
async function main() {
  ...
  const result = await request('session/prompt', {sessionId: s.sessionId, prompt: [{type:'text', text: 'Run the shell command: curl -sI https://example.com  (use curl directly, do not explain, just run it now)'}]})
