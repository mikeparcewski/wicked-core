#!/usr/bin/env node
// OQ-OPENCODE-ACP-001 evidence capture harness.
// Minimal ACP JSON-RPC-2.0-over-NDJSON client speaking directly to the installed
// `opencode acp` server. Captures every frame verbatim (both directions) to an
...
// Modeled on .product/evidence/oq-copilot-acp-001/capture-harness.mjs.
...
// Extra args appended to the spawn (e.g. --auto) to probe whether a startup flag
// changes ACP-mode behavior. Space-separated in argv[6].
const EXTRA_ARGS = (process.argv[6] || '').split(' ').filter(Boolean)

if (!OPENCODE_BIN || !FIXTURE_CWD || !CAPTURE_FILE) {
  console.error(
    'usage: capture-harness.mjs <opencode-bin> <fixture-cwd> <capture-file> <allow|reject> ["extra --flags"]'
  )
  process.exit(2)
}
...
const child = spawn(OPENCODE_BIN, ['acp', '--cwd', FIXTURE_CWD, ...EXTRA_ARGS], {
  cwd: FIXTURE_CWD,
...
async function main() {
  await request('initialize', {
    ...
    // client ANSWERS session/request_permission, so an opencode ACP server that gated
  ...
  const promptText = [
    ...
    `3. Run the shell command: echo hello-from-opencode`,
