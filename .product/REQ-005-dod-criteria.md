...
| L2-4 | Cross-language round-trip: `Core::launch_run` / `Core::subscribe` / `Core::confirm_gate` callable from TypeScript with correct event delivery | `tests/bus_bridge.rs` cross-language round-trip test exits 0 | ✓ (split evidence) — `run_requested_event_launches_a_run` (no `#[ignore]`) passes in CI: a bus-emitted event drives a run to `SessionCompleted`. `cross_language_roundtrip` carries `#[ignore = "requires node + sibling wicked-bus"]` and is NOT run by `cargo test` in CI; it must be verified manually with `cargo test -- --ignored`. TypeScript napi surface (`launchRun`/`subscribe`/`confirmGate`) verified via L1-6 (napi bindings ship for all 5 targets). |
...
| L2-1 through L2-7 integration tests all pass | ✓ L2-1,2,3(structural),4,5,6,7 verified; L2-5 cursor assertion confirmed. L2-4 note: `cross_language_roundtrip` is `#[ignore]` (requires node + sibling wicked-bus); non-ignored `run_requested_event_launches_a_run` passes in CI; TS napi surface verified via L1-6. |
