...
- **The T8 engine bindings: the catalog, a plan preview, a mid-run plan edit (DES-TEAMING-002 T8 (c), (e)).** Three commands crew's team routes call (they answered 501 until now). **`Core.catalog()`** (`catalog_entries()`, `src/catalog.rs`): the thirteen entries in catalog order as `{id, kind, role, gate, gate_type, executes_code, executor, validator_pin, pinned, evidence_floor, skill_ref, description}`. **`Core.previewPlan(planJson, projectId?, humanConfirm?)`** (`preview_plan`, `src/plan_gate/preview.rs`): the launch's own `precheck` → `intent_score_for_run` → `decide` over a fresh plan state, nothing persisted or published, read back as `{score, deterministic, reasons, destructive, band, high_risk, floor, floor_override, steps, def, pauses, pause_reason}` (the floor-fill fields are `plan::FloorFilled`'s; `decide` now returns the fill it decided on). A preview has no worktree, so it scores as a repo-less launch. **`Core.proposePlan(runId, planJson, requestId)`** (`Core::propose_plan`, `Command::ProposePlan`): the steps to add are held on the plan state (`TeamPlanState.edits`) and applied at the next step boundary by T4's `apply_held_revision`, after the PA's `PLAN+` lines, so the ratchet and floor fill apply (the edit only adds). Like an edit at the gate, its author approved it: no `plan_approval` gate opens for it and the rev is `plan.accepted{by:"human"}`; the PA's and agents' revisions still follow the approval matrix. The engine publishes `plan.proposed{by:"human", kind:"edit"}` sourced by the request id; a repeat request id (`TeamPlanState.edit_requests`) holds and publishes nothing and answers `{proposal_id, duplicate: true}`.
