...
- **The PA scopes a plan that declares no scope (DES-TEAMING-002 rev 13, seam X1).** A launch plan with a creator step (`build` / `produce`) and no declared `touch` — every preset, and a user plan that left `touch` out — is no longer scored 100 at launch (`"no declared scope"`), which put every preset launch, the default `feature` one included, in the top band with a `plan_approval` pause even in auto mode. Its plan rev 1 is now one READ-ONLY step, `pa-scope` (catalog `understand`, the PA seat), accepted by the engine without the matrix and never paused by the run-level policy. The PA ends its answer with one line (`plan_gate::scope`, parsed like `PLAN+`): `SCOPE {"touch":[…]}` on a repo — scored by the same scorer and blast-radius rules as a declared touch (no usable graph ⇒ 100) — or `RISK {"score":N,"reasons":[…]}` with no repo, which raises the lowest-band baseline (`THRESHOLDS.repo_less_baseline` = 0) through `path.scored.model.add` and can never lower it. At the scope step's boundary (`team_gate::apply_scope`, ahead of T4's revision hook) the scope step plus the launch plan go through `plan_gate::decide` as the INITIAL plan (`plan.proposed{by:<PA>, kind:"initial"}` → `path.scored` → floor fill → the matrix → `plan.accepted` or a `plan_approval` gate), before any creator step dispatches. A missing or malformed answer fails closed at 100 with the reason `"the PA declared no scope"`; a scoped plan that is refused or cannot be planned fails the run. `Core.previewPlan` of such a plan answers `graph: "pending_pa_scope"` (the baseline floor, the `pa-scope` step first, a "pending the PA's scope" reason) instead of 100 as if final. A floor override now needs a declared `touch` (refused at launch otherwise); a mid-run `propose_plan` while the PA is scoping is refused. T4's diff re-score still ratchets from the scoped band. On an UN-TEAMED repo run (no bus: the supervisor never re-scores its diffs) a declared scope could never be corrected, so its `SCOPE` fails closed at 100 (`"no diff re-score on an un-teamed run"`). Only the PA seat's answer counts (another seat's is not recorded, so it fails closed). A restart between the scope step's fold and its boundary's write never finalizes the run: the exec-mode re-drive routes it through `advance_or_pause`, and `finalize_run` refuses a run whose plan is still held. **Ord shift:** the `pa-scope` step is ord 1, so the preset's own steps start at ord 2 and a `human_confirm: before:<n>` names a unit one later than before X1. Follow-up for studio: its composer sends `before:N` in its own step numbering (`wicked-studio src/components/ChatInput.tsx:583`), which now lands one unit early on a scoped launch; and `previewPlan`'s `pending_pa_scope` floor is the baseline's, not the run's, so it must not be drawn as the real floor.

- **`Core.catalog()` serves each entry's `verified_evidence`.** `CatalogEntry.verified_evidence` is the entry's `PhaseDef::verified_evidence` (`true` for `test` and `domain_coverage` today): a step of such an entry is an acceptance requirement of the run that contains it. With this, crew can declare a plan or preset run's acceptance from the steps the run contains instead of a registered def looked up by name (crew#683). Additive key.

- **The T8 engine bindings: the catalog, a plan preview, a mid-run plan edit (DES-TEAMING-002 T8 (c), (e)).** Three commands crew's team routes call (they answered 501 until now). **`Core.catalog()`** (`catalog_entries()`, `src/catalog.rs`): the thirteen entries in catalog order as `{id, kind, role, gate, gate_type, executes_code, executor, validator_pin, pinned, evidence_floor, skill_ref, description}`. **`Core.previewPlan(planJson, projectId?, humanConfirm?, repoRef?, deliverStepJson?)`** (`Core::preview_plan`, `src/plan_gate/preview.rs`): what the launch would compute, nothing persisted or published — the plan as the launch resolves it, its `precheck`, the intent score against `repoRef`'s code graph at the base a launch would start from (`repo::local_run_base`: the local remote-default tip, else HEAD — read without a fetch, and only for a behavioural touch set), `decide` over a fresh plan state carrying the launch's deliver step, and `check_def_plans` on the def (a plan the launch would refuse is the preview's error). JSON `{score, deterministic, reasons, destructive, band, high_risk, floor, floor_override, steps, def, pauses, pause_reason, graph}`; the floor-fill fields are `plan::FloorFilled`'s (`decide` now returns the fill it decided on); `graph` is `ready` | `not_needed` | `unavailable`. **`Core.proposePlan(runId, planJson, requestId)`** (`Core::propose_plan`, `Command::ProposePlan`): the steps to add are validated by a dry run of `revise` (compose, provenance, a step the plan already has), refused once the deliver step has started, then held on the plan state (`TeamPlanState.edits`) and applied at the next step boundary by T4's `apply_held_revision` — FIRST, before a held floor raise or the PA's `PLAN+` lines, which the approval matrix then judges against the human's rev. Like an edit at the gate, the human's edit is approved by its author (`plan.accepted{by:"human"}`, no `plan_approval` gate of its own); floor fill and the ratchet still apply. The engine publishes `plan.proposed{by:"human", kind:"edit"}` sourced by the request id; the answer is `{proposal_id, duplicate, band, high_risk, floor_added}` (what the edit does to the run, from its dry run); a spent request id (`TeamPlanState.edit_requests`) answers `duplicate: true` whatever the run's state; an edit is refused while a plan awaits approval (edit it at the gate); an edit taken but not applied (the deliver step ran, the revision could not be planned, the run ended first) publishes `plan.refused`.

- **The team supervisor on the bus, member steps, councils (DES-TEAMING-002 seam T6).** The monitors' host moves off the engine's event fan-out onto a bus cursor (`src/team/supervisor.rs`): at spawn it snapshots the bus tail, arms the engine's live teamed runs (`Command::LiveTeamRuns`), replays each from its `stream_floor` and then tails (§4.7, one mechanism); an attempt claimed before this process booted is dead, and its unaccepted findings are re-raised on the unit's next attempt as `finding.raised{carried_from_attempt}`. A run whose `path.started` has aged out is a stream gap: an attempt whose earlier rows are gone folds `final_pass:"stream_gap"`, which pauses. On an attempt's `step.completed` the supervisor runs the final pass — one final batch per member, re-confirmation (`finding.settled{superseded}`), the hold round (exactly one `finding.settled{held|withdrawn}` per unaccepted finding; silence is `held`), one council per unresolved HIGH (`council.called` with a transcript of the finding's own rows, then `council.ruled`; no seats, a failed call, a timeout or the `MAX_DISPUTES` cap are each a `no_verdict` with its reason) — and publishes `ledger.folded`, the fold of the attempt's rows plus its overlays. A fold that misses its deadline is never published: it is tombstoned (a spooled one by the cursor loop, before any lane drains). An S fact that never reached the outbox, or one still waiting in it at the fold, makes the ledger `stream_gap`; the worker's own timeout synthesis is `stream_gap` too while any fact of the run is still spooled. Finding ids now carry the enclosing anchor (git's funcname heuristic; a file-level finding keeps the pre-anchor id) and `finding.raised` its computed `line_key`. The attempt runner publishes the PA's `ADVICE` lines as `advice.answered`, its `HELP:` lines as `help.requested` (a member answers with `help.answered`), and on a review its `STEP` line as `step.reviewed`, all before `step.completed`; the step-boundary block now also renders the team's answers (`help.answered`, `council.ruled`, `change.requested`) that no later step of the seat was shown, and labels carried findings. The ACP carrier publishes `checkpoint.reached` and sources its steer from the attempt's `finding.raised{high}` rows, recording one `advice.delivered{channel:"acp_steering"}` per carried finding. **Member steps (§8.8):** an `owner:"team"` step of a team run is placed on a member seat (never the PA's; none ⇒ `NoEligibleSeat`), passes its own gate with the member as its creator, and counts only once the PA's review turn (the unit's next attempt, on the PA seat, no judge) answers `STEP <id>: ACCEPT`; `REJECT to:member` reworks it with the reason as its amendment, `REJECT to:pa` (or the third rejection) re-plans it onto the PA, and a member `HOLD` convenes a council (`trigger:"member_step"`): YES counts it, NO keeps the rejection, no verdict pauses `team_dispute`. **`team_dispute` (DES-001 §6.7, #13):** the fold withholds `gateDecided{allow:true}` + `unitDone` and the cursor stays on the unit; approve publishes the human's `gate.decided{kind:"team_dispute"}` (a required fact) and only then emits `resumed`, `gateDecided`, `unitDone` and advances — never a re-dispatch; amend reruns the creator; reject cancels. **Deleted:** `TeamCmd`, `TeamHandle`, `StepRunner::team_finish`, `spawn_supervisor` and its hold-before-attach buffer, the steer mailbox, the per-run `TeamPlan` parameter, and the CoreEvents `unitCheckpoint`, `monitorAttached`, `monitorFinding`, `adviceDelivered` and `workerAdviceResponse` (team communication is the `wicked.team.*` stream; crew relays it as `teamEvent`).
- **The plan approval gate (DES-TEAMING-002 seam T3).** A launch can carry a user-composed plan (`LaunchSpec.plan` / core-ts `planJson`: `{steps, touch?, override?}`, a step's `id` defaulting to its catalog id), and a launch naming a preset now goes through the same pipeline: `plan.proposed` → the intent score from the declared `touch` against the run's graph at its base commit (a creator plan with no `touch` scores 100, "no declared scope"; no usable graph fails closed at 100) → floor fill → compose into the per-run def `<run>:plan-1`, registered through `register_composed` (so the run is a TEAM run) → the §8.6 approval matrix. Manual mode (`humanConfirm` `all`/`before:N`) always pauses; auto mode (`none`) pauses when the plan is high risk (band 70-100 or a destructive signal) and otherwise publishes `plan.accepted{by:"engine"}`; a floor override is recorded in manual mode and refused in auto mode. The pause is `awaitingHuman{gate_kind:"plan_approval"}` (new `PauseReason::PlanApproval`, beside P1's `team_transport`) before the first not-yet-dispatched unit, keyed by `gate_id = g-<run>-<gate_seq>` from P1's one gate counter (`RunTeamState.gate_seq`, same batch as the pause); plan state on `AgentSession.team_plan`, so a restart keeps the gate open. `confirm_gate` reads the open row's `gate_kind` first: approve and an accepted `HumanDecision::EditPlan` (core-ts `confirmGate(…, planJson)`, accepted directly as rev n+1 with floor phases added) release the unrun cursor unit once, with no attempt bump; a refused edit publishes `plan.refused` and re-opens the gate under a new `gate_id`; reject cancels. **The facts go through P1's `TeamBus::publish` only** (no new CoreEvent): the launch's `plan.proposed` / `path.scored` follow the run's `path.started`; `plan.accepted` is P1's required transition, now filled from the accepted plan (band, `high_risk`, steps with provenance, `by`, override); the plan gate's `gate.decided` is the required resume fact (a failure past the bound pauses `team_transport`); `gate.opened` and `plan.refused` ride the FIFO; an un-teamed run publishes none and its gate works the same. A delivering run hands its `deliver` step with the launch (`LaunchSpec.deliver_step` / core-ts `deliverStepJson`, only with a plan or preset — otherwise refused): it is appended to the plan and puts `deliver` in the floor, so a delivering preset run stays one gated team plan instead of a separately composed def. core-ts gains the `Core.supportsPlanLaunch()` static for launchers to key on (napi ignores undeclared fields). **Behaviour change:** a preset launch (`feature`, user presets) is now floor-filled and, since a preset declares no `touch`, pauses for plan approval even in auto mode; the straight-through `Core::launch` refuses a plan or preset launch. `gate.opened{plan_approval}.reviewing_ord` is now nullable (a launch plan reviews no unit).
- **The worker-thread seam (DES-TEAMING-002 seam T5).** Every team unit's worker thread (in-process and the bus worker, one seam: `run_unit_and_judge`) publishes `step.claimed` before the turn and `step.completed` after it, both R facts through P1's `TeamBus::publish`. `step.claimed` is required: past the bound the attempt's runner lane is tombstoned first and the attempt runs un-teamed with the reason on its snapshot; if the tombstone cannot be written the step fails before its turn. The step-boundary injector reads the run's stream once per step and hands the worker a `[team advice]` prior-context block with every raised finding that has no `advice.delivered{outcome:"injected"}` on any channel (8 KB cap), publishing one `advice.delivered{channel:"boundary"}` row per rendered finding. The gate wait polls for S's `ledger.folded` of the attempt for at most `FINAL_PASS_BUDGET` (floored at 30 s in production) and, on timeout, synthesizes DES-001 §4.7's fail-closed ledger from the attempt's own rows without publishing it (`stream_gap` when the stream is unreadable or incomplete, which pauses). The snapshot rides `UnitEvidence.team` and is persisted on `WorkUnit.team` (ledger, transcript, `ledger_ref`, `ledger_source`); the actor merges it with its dispatch stamp (downgrade only; a teamed stamp with no ledger folds as `stream_gap`). The engine publishes `gate.opened`/`gate.decided{kind:"unit_review"}`, and a gate that approved a unit whose ledger pauses opens `gate.opened{kind:"team_dispute"}` and pauses the run `team_dispute`. `team::runner::render_for_gate` puts the ledger and a ≤16 KB transcript inside the judge's WORK fence and into an evaluator's prior context. The judge excludes every ledger author and corroborator (instance and cli key) on the inline pinned and default paths; the bus request carries `excluded_seats`, and a response that does not name a judge outside them is a DENY. `run_team` units gain `ledgerRef`, `finalPass`, `teamPause`, `findings`. No new CoreEvent.

- **Reliable team publishing (DES-TEAMING-002 seam P1).** `TeamBus::publish` (`src/team/publish.rs`) is the one wrapper every team publisher calls: a fact the bus refuses is spooled to `<state home>/team-outbox.ndjson` through the SAME writer as the emit outbox (`wicked_apps_core::emit::spool_to`, a `EMIT-DEADLETTER:` line on stderr; the record adds `idempotency_key`, `run_id`, `owner`, `ord`, `attempt`), retried on a bounded schedule (5 attempts at 1/2/4/8/16 s), and drained per publisher and run in emit order (FIFO: a `gate.decided` can never land before its `gate.opened`). A replayed line is re-parsed and its key rebuilt; a mis-keyed line is refused. Tombstones (`superseded` by key, `superseded_run` by run/owner/attempt) live only in the outbox and are honoured by the drain, `Core::replay_team_outbox` and live publishes alike; a `path.ended` appended after a run tombstone still goes. The engine's facts go through the `TeamPublisher` thread, which acknowledges each to the actor (`TeamPublished` / `TeamTransportFailed` / `TeamSuperseded`); the actor never opens the bus and defers the gated step (`Progress::Deferred`). Required transitions fail closed: a team run dispatches nothing before its `path.started` is acknowledged — past the bound the run tombstone is written and only then `transport: none` is persisted (`AgentSession.team`, and `transport: none` stamped on every unit's snapshot at dispatch); `plan.accepted` past the bound pauses the run `gate_kind: team_transport` before the plan's first unit; that pause answers approve (retry, carried by the gate's own required `gate.decided` — failing again keeps it paused, no `Resumed`), approve with amend `"continue without team"` (tombstone, then un-teamed) or reject (tombstone, cancel, only `path.ended` published). No bus at boot = every team run un-teamed with `ledger_source: no_bus`, nothing spooled. Boot reconciles a live team run with no acknowledged `path.started`: tombstone first, then `transport: none`, before the publisher drains. No new CoreEvent. Reads: `Core::run_team` (core-ts `runTeam`, the state behind crew's `GET /api/v1/runs/:id/team`), `Core::live_team_runs` (never lists an un-teamed run), `Core::replay_team_outbox` (core-ts `replayTeamOutbox`). `team-outbox.ndjson` joins the state-home registry. Every §4.8 row has a named fixture (`row1_…` – `row12_…`).

- **Presets (DES-TEAMING-002 seam C2).** A preset is a saved phase selection: a named `steps[]` over the phase catalog. It is stored in the core store as a `Node(Other("plan_preset"))` row with `{name, scope: "global" | "project:<id>", steps[], created_by, updated_at}` (`src/preset.rs`). You write presets with `Core::put_preset`, `delete_preset` and `list_presets`, which are commands through the actor. `put` refuses a bad name, steps that do not compose over the catalog (`preset_invalid_steps: <catalog reason>`), an unknown project, and a global write to a built-in's name (`preset_builtin_readonly`). `delete` leaves a tombstone, because the store has no node deletion, and refuses a built-in. `list` returns the global rows, with a project's rows replacing them by name. The built-in presets are code data (`catalog::builtin_presets`). They are written to the store at actor boot, idempotent by name, with `created_by: "builtin"`. This PR seeds one built-in, `feature`, which is C1's §11.2 mapping and pinned against `tests/fixtures/catalog/mappings.json`. Each other consumer's preset lands with its M-seam. **`LaunchSpec.workflow` now names a preset first:** the launch's project row, then the global row, composed over the catalog, and only then a registered def. **Known behaviour change, pulled forward from M1: a `feature` launch now puts its `test` and `review` units on the evaluator role (they were neutral)**, because `feature` now resolves to its built-in preset, which is the catalog composition with §11.2's two bold cells. These units are therefore routed to an evaluator seat distinct from the creator where the roster has one. The rest of `feature` is unchanged: its phase ids, order, kinds, gates and pins. A name with no preset still launches its def, and an unknown name is still refused. The campaign driver and the bus launch bridge reach the same resolver. **core-ts:** `putPreset(name, stepsJson, projectId?, createdBy?)`, `deletePreset(name, projectId?)`, `listPresets(projectId?)`.

- **The phase catalog and `plan::compose` (DES-TEAMING-002 seam C1).** `src/catalog.rs` holds the twelve phase types of §8.3 as `PhaseDef` data (`understand`, `test_plan`, `design`, `architecture`, `build`, `produce`, `test`, `review`, `critique`, `security_review`, `run`, `deliver`). The evidence-floor pin now sits on the catalog entries: `build`, `test`, `review` and `security_review` carry it. `security_review` runs the garden QE security specialist, `wicked-garden-qe-security-test-engineer` (Q6). `plan::compose(catalog, &PlanSteps)` is the only constructor of a per-run def. A step may only make its entry stricter: raise the gate, add or swap a pin, or set `executes_code`. Every step field is classified once, in `STEP_FIELD_RULES`. `role` is fixed and `kind` may be set only on `run`. `gate`, `validator_pin` (add or swap, never remove), `executes_code` and `required_deliverables` (may only grow) are tighten-only. `instructions`, `skill_ref` and `allowed_skills` may be set only on an entry that leaves them unset, so a `security_review` step cannot swap in a non-security skill. `executor` is allowed only on the Tool entries, which require it. `gate_type`, `depends_on` and `owner` are free. A loosening step is refused with a named `PlanRefusal` reason, and restating the entry's own value is a no-op. The composed def is then judged by `WorkflowRegistry::register`. `PhaseDef.owner` / `WorkUnit.owner` (`StepOwner`: `pa` by default, or `team`) is copied onto the unit the same way `role` is. It is skipped on the wire when it is the default, so every shipped workflow file and every persisted unit serializes byte-identically. `tests/catalog_compose.rs` composes all 19 §11.2 consumers. Each composed def equals today's def per phase except §11.2's eleven bold cells, and migration cleanup is pinned as `build`. Nothing launches through the catalog yet: no consumer is migrated and no workflow file is deleted.

- **T0 bus handoff, engine half (DES-TEAMING-002 T0, rev 11).** The engine can now be handed a bus on every boot without changing what a default daemon does. **(1)** The gate judge's bus round-trip (`wicked.gate.eval.requested`, 180 s fail-closed) is used only over a bus exec mediation actually ARMED for the unit (passed by the caller), never from process env; before this, the variable by itself rerouted every evidence-floor-pinned judge to a responder no product runs. **(2)** The actor never opens the bus. `Core::spawn*` arms the launch bridge on the caller's thread with a bounded handshake (`BUS_ARM_TIMEOUT`, 2 s): the bridge thread opens the bus and reads `MAX(event_id)`, and `spawn` returns only once the bridge polls from that event-id floor — so a request emitted after `spawn` returns is delivered and a row already on the bus is never launched, whatever its `emitted_at`. A bus that does not answer in time leaves the bridge not armed (`Core::bus_bridge_state`, core-ts `busBridgeState()`), never a guessed start point. Exec mediation's init runs on a bus thread, so a locked bus no longer stalls the first command by the 5 s busy timeout. **(3)** One connection per bus file per process, never closed (`BusDb::shared`; `BusDb` clones share it; a bus file recreated under the same path — a different file identity, `(dev, ino)` on unix, creation time on Windows — gets a fresh handle and the old one is leaked), because crew's better-sqlite3 seams share the file in the same process and any close drops the process's POSIX locks (the F-E2E-021 class). A source-guard test fails the build on a production `BusDb::open`. The armed bridge has one owner: dropping it stops and joins its thread, so an actor that returns early (no store) leaves no poller running. The pinned judge follows the bus exec mediation ARMED for the unit (the cli-runner's own bus, the actor's armed publisher's) and never re-reads process env, so the env-free `Core::spawn_with_engine_exec` entry judges over its bus, and `WICKED_BUS_EXEC` with a bus exec cannot initialise keeps the in-process judge inline instead of denying on the missing bus. **core-ts:** `Core.busConnectionStats(path)` reports the one connection; crew keys the handoff on its presence.

- **Teaming S2: read-only monitors watch a running unit at tool-call checkpoints (#601, DES-TEAMING-001 §4).** Three additive CoreEvents, nothing else on the wire changes. `unitCheckpoint {session, ord, attempt, seq, toolCallId, kind, title, status, paths}` is emitted by the ACP carrier at each TERMINAL `tool_call_update` (`completed` | `failed`) of a TEAMED unit — `kind`/`title`/`locations` remembered from the earlier `tool_call` frame, since the terminal one repeats none — and never for a non-teamed unit or per token. A per-daemon supervisor (`src/team.rs`) subscribes to the engine's own fan-out and batches: a monitor starts at most one batch per 60 s, only after a tree-changing checkpoint (`read`/`search`/`fetch`/`think` never count), never while one is in flight, at most 10 per attempt (exhaustion is `monitors[].status: "budget_exhausted"`); each batch snapshots the worktree once through the pinned git dir, and an unchanged tree id costs no session and no model turn. A monitor is a warm READ-ONLY ACP session on a seat instance distinct from the creator (`claude#2`, via #605), admitted only when its adapter is `acp_input_governance: true`, judged by the chat boundary (its private scratch root is the only write root; the worktree is a read root) and emitting no chat events; a refused candidate is `monitorAttached {status: "failed", error}` and starts no process. Every `FINDING` a monitor replies with is checked mechanically — strict parse, the `high`/`medium` bar, `evidence` equal to line `line` of `path` in the snapshot tree — and deduplicated on the line's TEXT (`findingId = f-` + 16 hex of sha256(path, normalized evidence)), a second monitor recorded in `corroboratedBy`; only survivors are emitted as `monitorFinding`. `StepRunner::team_finish` (default `None`) runs the bounded final pass on the settled tree — summoning monitors for a unit that had none (the wrapped carrier), re-confirming every finding (`finalLine`, or `superseded`) — and returns the `TeamLedger`. Advisory only: no injection (S3, #602), no gate input (S6, #603), no fold change. The monitor count and candidates are a plain `TeamPlan` parameter (`AcpStepRunner::set_team_plan`) until S4/S5 land, so no production unit is teamed yet.

- **Per-phase distribution no longer convenes a council; a council now rules only on a concrete disputed decision (#590 S5).** Each seated unit takes the first eligible seat its skills admit, in roster order, and records the new `RoutingInfo::Teamed { winner }` (wire `{"method":"teamed","winner":"<seat>"}`; `unitDistributed.routingMethod: "teamed"` with `agreementPct`/`returned`/`seated`/`dissent` `null`). No ballot is dispatched and no council event is emitted to route a unit. The evaluator≠creator fence (`enforce_evaluator_distinct`) is unchanged: a review/test unit on a builder seat still moves to a non-builder seat (`evaluator_distinct`), is disclosed (`creator_seat` / `same_cli_instance`) on a bench-free roster with no distinct seat, and is refused (`NoEligibleSeat`) when a bench empties the pool. The routing council's code is deleted, not flagged off: `distribute_one`, the verdict parser, the ballot ledger (`SeatLedger`, the ballot-bench rules) and the reassign that bench drove; the `dispatcher`/`relay`/`operational_home` parameters leave `distribute_units_on*`, `plan_and_distribute`, `run_session`, `launch_run_inner` and the campaign seams. An operator reassign (`{cli: null}`) re-routes over the WHOLE plan so the fence still sees the run's builders. The one council entry point is `Core::convene_decision(DecisionRequest { session_id, ord, question, options, evidence }, clis) -> DecisionVerdict { winner, consensus, agreement_pct, returned, seated, dissent, no_ruling_reason, task_id }` (`src/decision.rs`), which reuses `wicked_council::Worker`, the engine's injected dispatcher (production `RealDispatcher::from_env`, load-scaled ballot budget) and the claude-ballot deny fence; nothing calls it yet (S6 will). `Council` and `Degraded` stay in `RoutingInfo` so recorded runs still load. core-ts: `UnitDistributedEventJson.routingMethod` gains `'teamed'` (additive; wicked-crew's contract must accept it first).

- **The repo-checks floor now runs `cargo fmt --all --check` before `cargo test` for any Rust workspace, matching CI step one (core#551).** Every governed Rust delivery was exposed to a CI failure at the first step — `cargo fmt --check` — even though the floor passed: a formatting drift that CI caught on all three OSes was invisible to the floor because the floor detected only `cargo-test` from `Cargo.toml`. The formatter check is now auto-detected from `Cargo.toml` (the same signal the test check used) and runs first, before tests, so a creator sent back to format gets the denial with the diff tail rather than reaching CI. `cargo fmt --all --check` is used to match CI exactly and works for both workspace and single-crate manifests. A repo can declare `"formatter": false` in `.wicked/checks.json` to disable it, or supply an explicit command; the existing `"test"` override does not touch the formatter slot. The fix also covers core#547 (the formatter was not expressible in `checks.json`). Two new tests pin the behaviour: `unformatted_rust_fails_fmt_floor` (red on origin/main, green after) and `formatted_rust_worktree_passes_fmt_floor` (control, must stay green).

- **A seat instance now runs on the ACP carrier in its own configuration home (#591, the ACP-spawn KNOWN GAP; prerequisite of DES-TEAMING-001 S2).** On `d5d9708` a `claude#2` unit that reached the ACP spawn found no registry record — `registry_record` in `src/acp_runner.rs` matched the exact key only — and fell back to the wrapped carrier; and had it found one, `start_acp_process_with_write_roots` resolved the configuration root with `seat_config_for(seat_cli)`, the PRIMARY instance, so it would have run in `<base>/claude` beside the creator. Both reproduced before the fix. `registry_record` now applies the same two-step `[cli_key, seat_cli_key(cli_key)]` lookup the wrapped carrier uses (an exact record under the instance key still wins), and the spawn takes the roster seat key and resolves `seat_config_for_seat(seat_cli, seat_key)`: a secondary claude instance runs with `CLAUDE_CONFIG_DIR = <base>/claude-2`, created, re-sanitized and fenced by the one writer the council ballot already uses for that home (`ensure_secondary_instance_fence`, #595 — no second copy). The primary `claude` seat is unchanged. The codex skills-home population and the not-authenticated login hint read the same instance-keyed root. `a_seat_instance_key_resolves_its_clis_record_on_the_acp_path` and `a_secondary_claude_acp_worker_runs_in_its_own_fenced_sanitized_instance_home` failed on `d5d9708` and pass now. Per-instance sign-in (wicked-crew#615) is unchanged: a `claude#2` home still needs its own login.

- **A worker's `AskUserQuestion` now reaches the decider instead of being auto-cancelled (#594).** We advertise `elicitation: {form: {}}` to `claude-agent-acp`, which is what enables the model's `AskUserQuestion` tool, and then `validate_elicitation_schema` cancelled every request that tool produces: it required exactly one property (the adapter always sends two per question, `question_0` plus a free-text `question_0_custom` companion), read options from `enum` (the adapter uses `oneOf`) and rejected multi-select `type: "array"`. The validator now accepts the adapter's real shape for one question: the `<field>_custom` companion is recognised and left unfilled, single-select options are read from `oneOf`/`anyOf` `const`s (plain `enum` still works), and a multi-select `array` surfaces its `items.anyOf` options with the chosen option delivered as a one-element array. The question rides the existing path (`ElicitationCreated` to crew's `/runs/:id/elicitation` to studio's prompt to `resolve_elicitation`) and the answer goes back under `question_0`. The same `oneOf` read also gives the adapter's refusal-fallback dialog its two options where it used to fall through to free text. Shapes the one-answer surface cannot carry still fail closed, and a cancel now says why in the turn output: a request with more than one question, a non-string/array type, an option list that is not all strings, or an array with no option list. A `_custom` field only counts as the companion when the adapter's `_meta._askUserQuestionCustomAnswer` marker names its base field, and options that registration would drop (empty, or over 512 bytes) do not count, so a select left with none cancels rather than surfacing an unanswerable prompt. Pinned by `tests/fixtures/claude_agent_acp_0_73_0_ask_user_question.json`, which is the adapter's own `askUserQuestionsToCreateRequest` output, replayed through a mock bridge that asserts the exact accept payload.

- **Three bugs on the secondary-instance path are closed (#595).** All three were present on `c163cf9` and reproduced before each fix. **(1) Wrapped carrier resolved the primary config home for every instance.** `seat_config_for_carrier(&binary)` passed an empty seat key, so `seat_config_for_carrier_seat` always resolved `<base>/claude` — a `claude#2` wrapped worker shared the primary worker's config home. The call site in `exec` (`execute_wrapped.rs`) now passes `&cli_key`, so `claude#2` resolves `<base>/claude-2`. `a_secondary_claude_wrapped_worker_uses_its_own_instance_home_not_the_primary` reproduces and pins the fix. **(2) Registry lookups in `seat_posture_from` and `worker_os_sandbox_enabled` matched on the exact key only.** `codex#2` found no registry record, `trust_flags` defaulted empty, and the OS-sandbox flag was silently read as `false`. `resolve_invocation` in the same file already applied the two-step `[cli_key, seat_cli_key(cli_key)]` loop; both functions now mirror it exactly. Sweep over every `.find(|c| c.key == …)` in `execute_wrapped.rs`: only these two lacked the fallback (the remaining sites operate on roster membership or engine-internal constant keys). `secondary_instance_inherits_trust_flags_from_its_cli_registry_record` reproduces and pins the fix. **(3) A secondary claude ballot's instance home (`<base>/claude-2`) was unfenced.** `ensure_shared_worker_fence` (in `acp_runner`) writes only `<base>/claude`; the council dispatch for `claude#2` ran under `<base>/claude-2` without a `settings.json`. New `ensure_secondary_instance_fence(seat_key)` in `execute_wrapped.rs` writes the identical shared deny rules to the instance home (no-op for primary instances and under the inherit hatch); `distribute_units_on_benched` calls it for every secondary claude ballot before the roster is handed to the council — same fail-closed pattern as `ensure_shared_worker_fence`. `acp_runner.rs` and `actor.rs` are untouched. `convening_a_secondary_claude_ballot_writes_fence_to_the_instance_home` reproduces and pins the fix. The KNOWN GAPS note in the #591 entry is updated: the wrapped-carrier path IS now wired for secondary instances; the ACP spawn path is wired since the follow-up below, and `next_failover_seat` is the one gap that still applies.

- **A seat key is now an INSTANCE identity, so one cli can hold more than one seat — with its own configuration home and its own disclosure (#591, seams S1–S3).** A run's worker pool was capped at one seat per cli, so parallelism was bounded by how many vendors were signed in and a degraded seat could not be replaced by another instance of a healthy one. Two identity collisions made it so, and BOTH were reproduced on `origin/main` before anything changed. (1) `enforce_evaluator_distinct` builds `builder_clis` as a `HashSet<String>` of `assigned_cli`; two records spelled the only way main's roster could spell two instances — the SAME `key` — collapse to one entry, so a review unit found no seat distinct from its builder and stayed on it (`same_seat == [2]`). (2) `invocation_of` resolves with `.find(|c| c.key == key)`, so those two records resolved silently to the FIRST template and the second instance's own was unreachable. **S1**: a seat key is `<cli>` or `<cli>#<instance>` (`claude#2`), the cli key is its prefix (`wicked_apps_core::spawn::seat_cli_key`) — a derived attribute rather than a second roster field, which keeps the mapping total for an operator's own `my-claude` and keeps the configuration-home derivation injective. Routing, `assigned_cli` and the wire carry the INSTANCE; the predicates that are about the MODEL read the cli key: `seat_is_claude` now asks both identity resolvers about `claude`, not `claude#2`, which would have found no registry record and barred every second claude instance from every Claude-only review unit. A roster that names one key twice is REFUSED at plan entry (`refuse_duplicate_seat_keys`, before any routing, including for an all-tool plan) rather than disambiguated — an identifier that names two seats is not an identifier — and the refusal names the key and the `claude#2` remedy. **S2**: the whole seat-configuration decision is keyed on the instance, so `claude#2` resolves `<worker home>/claude-2` and EVERY configuration-home variable (`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `PI_CODING_AGENT_DIR`, `COPILOT_HOME`, the XDG trio, `HOME` for agy) is aimed inside that root. This is a PATH change only: it moves where a cli reads its configuration from and touches no credential store. The instance suffix is `[A-Za-z0-9_]+` and an unacceptable one is REFUSED, never sanitized — a sanitizing map would fold `claude#2` and `claude#/2` onto one directory, which is the isolation silently undone — and `refuse_symlinked_home` and `hardened()` are unchanged: a planted link at an instance root is refused exactly as at a base one, constructed and asserted rather than assumed. The council ballot spawn passes the seat's key, and `default_login_invocation` derives the COMMAND from the cli and the DIRECTORY from the instance (choosing both on the whole key would leave every second instance with no sign-in surface; resolving the root on the cli key would send an operator to sign in a directory no second instance runs under — the F-013/F-010 mismatch that derivation exists to prevent). **S3**: `unitDistributed.distinctnessFallback` gains a second value, `"same_cli_instance"`, emitted when a review/test unit IS on a seat distinct from every builder seat but that seat runs a builder's cli (`claude#2` grading `claude#1`). Instance distinctness removes the creator's CONTEXT, not the model's blind spots; accepting it silently as model-distinct is the silent degradation this field exists to prevent. `"creator_seat"` dominates when both would apply, so a one-seat roster still discloses the stronger failure. The closed token set is corrected in all four places it lives, together: the Rust event doc (`src/event.rs`), the core-ts key list (`crates/wicked-core-ts/src/lib.rs`), the PUBLISHED `.d.ts` and the GENERATOR that emits it (`scripts/finalize-dts.mjs`, edited then REGENERATED, not hand-edited), plus the compile-time assertion in `types-test/`. One regression was caught and pinned on the way: threading the seat key through `seat_config_for_carrier` resolved the worker home base EAGERLY, which began refusing every non-claude ballot under a relative `WICKED_WORKER_HOME` (a cli with no configuration home of its own must not be refused for a base it does not use); the base is now resolved lazily and `a_rootless_cli_is_decided_without_resolving_the_worker_home` pins the ordering at the resolver. **KNOWN GAPS, stated rather than implied.** The worker SPAWN path (`src/acp_runner.rs`) resolves a seat's registry record, ACP config and configuration root by the key it is handed, so a `claude#2` unit would find no registry record and fall back to the wrapped carrier with the roster's own template; and `next_failover_seat` (`src/actor.rs`) excludes creator SEATS, so a failover onto another instance of a creator's cli is instance-distinct but emits no `same_cli_instance` disclosure. Both files were being changed concurrently by the ACP elicitation re-land and are deliberately untouched here; wiring them is the remainder of #591 alongside S4 (pool policy) and S5 (per-instance login, wicked-crew#615). The wrapped-carrier path is now wired (#595) and so is the ACP spawn path (the #591 ACP follow-up above); `next_failover_seat` remains the open gap.

- **A seat whose `verified_version` pin no longer matches the installed build now says so on every turn that runs on it, naming both builds (#581).** The pin fails a drifted seat's ACP input governance closed, by design - but the only disclosure was a per-turn `governanceUnenforced` emitted under `gate_ctx.is_some() && !proc.governance_verified`, i.e. only for a GOVERNED unit, and its message named neither the pinned nor the installed version, so an operator had to find the constant and run `--version` by hand to learn what had drifted. An ungoverned unit said nothing at all. The spawn-time probe now keeps what the binary reported (`VersionPinProbe { matched, observed }`), an `AcpProcess` carries the mismatch as `version_pin_drift`, and ONE disclosure site - placed after the read-only reroute, so a unit sent to the wrapped carrier never claims to have run unchecked - emits `governanceUnenforced` for every turn that actually starts on the drifted bridge, governed or not. A governed turn adds the per-unit consequence. The reason names the pinned build, what the binary reports (or that it did not answer `--version`, or that it printed the version somewhere other than the first line, which is what the pin compares), the configured binary and where it resolves (distinguishing a bare name that is not on PATH from an absolute path that does not exist), and what to do. A seat that claims no input governance discloses nothing: its pin gates no admission, and such a seat never reaches a gated turn. The `matched` comparison itself is deliberately UNCHANGED (still the first line, byte-for-byte after trimming): reading further for the disclosure is a readability fix, and widening what counts as a match would loosen the admission gate. KNOWN GAP, stated rather than implied: the CHAT path (`chat_open` / `chat_ensure_one`) spawns through the same function but emits no `governanceUnenforced`, so a drifted seat still opens a chat silently. The event is unit-shaped (`ord`, `attempt`); wiring chat needs a decision about what those mean there, or a different event. Wire shape unchanged - no new event type, no new field; `governance_verified` keeps its meaning. NOTE: the opencode pin is deliberately LEFT at `1.17.18` although `1.18.31` is what installs today. The pin's value is entirely what it was proven against (oq-opencode-acp-002); moving it without re-running that proof would assert a verification nobody performed. Re-proving 1.18.31 remains open on #581, as does carrying the date/method of proof alongside the pin.

- **The dead same-seat `degradedReason` disclosure is removed and three stale comments in `distribute` now say what the code does (#567).** `distribute_units_against_benched` carried an F-7R3-001 disclosure that appended an "evaluator≠creator not enforceable ... stays on creator seat" note to `degradedReason` under `!benched.is_empty() && same_seat.contains(&u.ord)`. core#560 replaced that disclose-and-continue policy with a refusal: the guard above it returns `NoEligibleSeat` whenever `!same_seat.is_empty() && !benched.is_empty()`, so everything past it satisfies `same_seat.is_empty() || benched.is_empty()` and the disclosure's condition is false either way. Neither binding is mutated in between, so the branch was unreachable - confirmed mechanically: an `unreachable!()` planted in it was not hit by any of 1074 lib tests, while the three existing fail-closed tests (one per bench source: launcher, ballot, worker) construct precisely the state it described and all get `Err(NoEligibleSeat)`. It is deleted rather than widened into reachability: the refusal is the safety property (a benched roster must not let a seat grade its own work), so a reachable disclosure would mean regressing #560. NO BEHAVIOUR CHANGE - the only non-comment lines removed are the dead branch. The three comments that described the removed behaviour are corrected: the loop header (which advertised the same-seat disclosure), the `distinctnessFallback` comment (which said only the "ballot-bench case" was fail-closed, when all three sources are since #560), and rule 2 of the function's doc header (which said a review/test unit "stays on its creator seat" with `degraded_reason` saying so when a bench emptied the pool - it is refused instead). The surviving reachable path is unchanged: a BENCH-FREE roster with no seat distinct from the builders keeps `degradedReason: null`, the gate's UNGATED disclosure, and `distinctnessFallback: "creator_seat"`. That path is wider than a too-small roster and the comments now say so - it is also reached by a roster of two or more seats where every seat was assigned a Build/Recon unit, and by one whose only non-builder seats the unit's skills refuse. Only the all-seats-built shape ALSO warns on stderr: `warns_about_missing_evaluator_seat` requires `roster_keys.len() >= 2`, so a one-seat roster is silent by design (pinned by `a_single_seat_roster_is_unchanged_and_never_warned_about`). The same stale claim - that a bench emptying the pool sets `distinctnessFallback`, with `degradedReason` naming which - lived in three places, and all three are corrected together: the Rust event doc (`src/event.rs`), the PUBLISHED TypeScript surface (`crates/wicked-core-ts/index.d.ts`) and the generator that emits it (`crates/wicked-core-ts/scripts/finalize-dts.mjs`). The generator was edited and the `.d.ts` REGENERATED from it, not hand-edited, so the next build cannot reintroduce it; the existing `hand_authored_dts_is_in_lockstep_and_declares_every_unit_distributed_key` test is what holds those two together.

- **The tool-child registry now tracks every live tool-child pgid instead of overwriting (#577); the leader-exit orphan of #575 does not reproduce.** `ElicitationMaps.tool_child_pgroups` was `HashMap<String, u32>` and `register_tool_child` overwrote unconditionally, on the premise that one live tool child per run is an invariant. It is not. `ReassignUnit` advances the launch sequence, drains the registry with `take_tool_child_pgroups`, and dispatches the replacement while a superseded attempt sits in the window between `proc.spawn()` and its own registration: the drain finds nothing and two tool children are live at once. Nothing orders their two registrations — the superseded attempt's worker thread only has to be off-CPU across the replacement's dispatch and spawn — so the SUPERSEDED attempt can register second, and the overwrite then evicted the *replacement*, the live one. Its own `on_done` next deregistered its own pgid, whose compare-and-remove matched and emptied the run's entry, so a later cancel took `[]`, reported `toolChildrenKilled: 0`, and left a live tool child running that nothing could reach — the exact escape #500 closes (a cancelled run's deliver script committing and pushing under the daemon's token). The storage is now `HashMap<String, Vec<u32>>` and holds what IS live rather than what is expected to be: `register_tool_child` appends (idempotent on a repeat of the same pgid), `deregister_tool_child` removes only the caller's own pgid and leaves any other live one in place, and `take_tool_child_pgroups` returns them all — the callers already loop `for pgid in pgroups { kill_pgroup_graceful(pgid) }`, so a cancel during the race now kills both groups. `register_tool_child` is `#[must_use]` and returns the pgids that were already live for that run (empty in the ordinary case); `on_spawn` logs when it is not empty, because co-residency is rare and worth knowing about even though it is now handled. Deliberately not a panic: a worker-thread panic while holding the lifecycle mutex would be worse than the condition it reports. **On #575** (a tool child's pgroup deregistered when its LEADER exits, orphaning backgrounded work): probed against the real `run_tool_cmd`, three runs, and the backgrounded process is already dead when the call returns — the `Ok(true)` arm kills the whole group before collecting the status, while the un-reaped zombie leader still reserves the pid that is the group id, so the group kill lands before `on_done` deregisters anything. The premise does not hold on unix and nothing is changed for it; a regression test (`a_natural_leader_exit_still_takes_everything_it_backgrounded_with_it`) now pins that ordering, since the group kill on an already-exited child reads like a redundant line.

- **The 500 ms graceful SIGTERM window is now real: the tool worker no longer SIGKILLs the child out from under the actor's kill (#576).** SUPERSEDES the "Do not rely on the 500 ms window" caveat in the #500 entry below — that caveat described this defect, and it is now fixed. `kill_pgroup_graceful` (actor thread) sends SIGTERM and polls up to 500 ms, but the background tool thread polls its child every 50 ms and, on the `Ok(false)` arm (child still running — possibly inside its own SIGTERM handler), called `kill_child_tree` → an immediate `killpg(pgid, SIGKILL)`. The group was therefore SIGKILLed within 0–50 ms of the cancel; the actor's poll then found the group gone and returned from inside its SIGTERM window, and a comment attributed a graceful death that had not happened. A tool child that flushes buffers, removes temp files, or stops its own subprocesses on SIGTERM was killed mid-cleanup. That arm now calls `validator::kill_after_stop(child, actor_kill_grace(reason))`: for the stop reasons the actor escalates for — `cancelled` (`cancel_run`: `CancelRun`, campaign cancel/fail-fast, human Reject, governance Deny) and `superseded` (`ReassignUnit`) — the worker polls `has_exited_unreaped` for up to `ACTOR_KILL_GRACE` (1 s, derived from the actor's 500 ms SIGTERM poll + 200 ms SIGKILL confirmation + slack) instead of signalling, letting the actor's SIGTERM land first. On `shutdown` — where `Command::Shutdown` only sets the flag and never kills a tool child — and on non-unix, the grace is `None` and the kill is immediate exactly as before. **The grace is a bound, not a leak:** after it (or the moment the leader exits) the worker still calls `kill_child_tree` + `reap_bounded`, so the whole group is force-killed and the child reaped before the phase returns — a cancel whose pgid was never registered still ends with a dead child, and background work the leader left in the group still dies with the phase. Worst-case added latency for a stop: 1 s, only when nothing else kills the group. The three stop words are now the named constants `STOP_CANCELLED` / `STOP_SUPERSEDED` / `STOP_SHUTDOWN` (same wire spellings) so the kill policy and the predicate cannot drift apart. `kill_pgroup_graceful`'s return value is documented as reporting DEATH, never its CAUSE — it holds no `Child` handle and cannot see a status; the `// died on SIGTERM` comment that asserted otherwise is gone. Wire shape: unchanged.
- **A campaign Reject now TAKES EFFECT instead of reporting success it did not achieve — and never leaves a stuck node (#574).** `campaign::confirm_gate`'s `CampaignGateDecision::Reject` arm discarded the `Result` of the inner `actor::confirm_gate` call with `let _ =`. `actor::confirm_gate` refuses unless `session.status == AwaitingHuman`, and the campaign's `node_status` can disagree with the session's status — nothing reconciles the node when the run leaves `AwaitingHuman` by a non-campaign route (answering the run-level gate directly via `Core::confirm_gate(run_id, Approve)` resumes the session to `Executing` and leaves the node `AwaitingHuman`). The outer node guard then passed, the inner session guard bailed, the error was dropped, and the operator was told `Ok(<campaign status>)` while the run kept executing: no `RunCancelled`, child still alive. Merely propagating that error is not the fix: it reports the failure but reconciles nothing, so the node stays `AwaitingHuman` with nothing able to move it — `on_node_awaiting` drops the run's later gates (it gates only a `Running` node), so the run can park forever, never reach `on_run_finished`, and `finalize_if_done` early-returns on `any_waiting` for good. The arm now tries the gate path FIRST (it records the human's answer on the durable prompt row as `Answered{"action":"reject"}`, which a bare cancel would overwrite with `Cancelled`); its status guard bails before any mutation, so a refusal leaves nothing half-applied. On refusal it does what the operator asked and what that arm does anyway — `cancel_run` directly, which carries no gate precondition, kills the tool-child group (#500), and drives the same deferred `CampaignRunFinished` → `on_run_finished` node reconcile. Which path is taken is decided on the session's PRIOR status, never on `cancel_run`'s return value: that call answers `Ok(SessionStatus::Cancelled)` both when it has just cancelled a live run and when the run was ALREADY `Cancelled` (its terminal early-return reports the status and, by design, does not re-emit a terminal event or re-notify the campaign), so the two cases are indistinguishable afterwards and an already-cancelled run would take the live-cancel arm — no reconcile, and an `Ok` claiming a denial that never happened. Every terminal prior status (`Completed`, `Failed`, `Cancelled`) and a missing session row instead reconcile the node THEMSELVES and then error, because none of them will produce a fresh terminal event for the reject to ride on — the same "never a stuck node" rule the launch/resume failure arm in `dispatch` applies. The status match is exhaustive with no wildcard arm, so a new `SessionStatus` variant is a compile error rather than a silent default. Behaviour change for API callers: `ConfirmCampaignGate` with `Reject` now CANCELS a run that has drifted out of `AwaitingHuman` where it previously did nothing, and can return `Err` (with the node reconciled) where it previously returned `Ok`. `denial_gate: auto_reject` is unchanged — it still logs and HOLDS. The underlying node-status drift (the second defect in #574) is NOT fixed here; this makes the drift survivable rather than silent or wedging.

- **Tool child process groups are now killed before `RunCancelled`/`UnitReassigned` are emitted (#500).** When `cancel_run` or `ReassignUnit` fires, the engine now synchronously kills the tool child's process group before the event leaves the actor — that ordering is the guarantee, and it holds on every path. The *manner* of the kill is best-effort, not guaranteed: the actor sends SIGTERM and polls up to 500 ms before SIGKILLing survivors, but the background tool thread owns the `Child` and polls every 50 ms, so it will usually observe the stop predicate first and call `kill_child_tree` — an immediate `killpg(pgid, SIGKILL)` with no grace period. The actor's poll then finds the group already gone and attributes it to SIGTERM. Do not rely on the 500 ms window: in the common case the child is SIGKILLed without it (#576). This closes the window where a cancelled run's deliver script could commit, push, and open a PR under the daemon's token. The `runCancelled` event gains a `toolChildrenKilled: n` field (count of groups killed); `unitReassigned` gains `previousAttemptReaped: bool`. A new `toolResultDiscarded` event fires (with `reason: "cancelled"` or `"superseded"`) when a killed or superseded TOOL attempt's result posts back after the run is already terminal; non-tool units never emit this event. The background tool thread checks the stop predicate in the `Ok(true)` arm of its poll loop so a group killed by the actor is never mistakenly treated as a natural exit; a check before spawn closes the pre-spawn window too. The launch-state invalidation (`tombstone_run` + `advance_launch_seq`) now lives inside `cancel_run` itself so all cancel paths — `campaign::cancel`, `campaign::fail_fast`, human Reject, governance Deny, and `CancelRun` — inherit it uniformly; the tombstone is no longer retired after cancel, so the tool thread always observes `reason: "cancelled"` (never `"superseded"`) on an operator cancel. Campaign dispatch (`campaign::dispatch`) now passes `lifecycle_maps` to `launch_run_inner` and `confirm_gate` so the tool child's pgid is registered from the campaign path, enabling `cancel_run` to reach and kill the group when `cancel_campaign` fires. The four remaining campaign call sites now pass it too — the `denial_gate: auto_reject` answer in `on_node_awaiting`, the operator's `CampaignGateDecision::Reject`, and both `campaign::resume` branches (fresh launch and re-attach) — where they previously passed `&None`, making `cancel_run` take its `None` branch and skip both the kill and the launch-state invalidation. What each pair actually delivers differs, and the distinction matters: the two **resume** sites restore pgid registration, so a later cancel can reach and kill the group; the two **reject** sites deliver only the launch-state invalidation (`tombstone_run` + `advance_launch_seq`), *not* a kill. A run is rejectable only while `AwaitingHuman`, and the engine never holds a registered tool child at a gate — `run_tool_cmd` calls `on_done(pid)` unconditionally when a unit ends, deregistering the pgid — so `toolChildrenKilled` on a reject path is always 0 by construction.

- **Evaluator≠creator guard now refuses for ANY bench source — launcher, ballot, or worker (#560).** The guard in `distribute_units_against_benched` previously checked `benched.iter().any(|b| b.source == "ballot")`, so a distinct seat benched by the launcher health probe or by a worker transcript fell through silently to the `creator_seat` fallback. The comment and CHANGELOG entry for #537 both claimed "Launcher-bench (source ≠ 'ballot') is handled by the existing `park_at_dead_seat_gate` path" — that path is reachable only when distribution returns an error, so it could not catch a silent fallback. The guard is now `!benched.is_empty()`, refusing for all bench sources. The refusal message names the source via the existing `benched_summary` (e.g. `"pi (unusable — launcher)"`). A bench-free roster that is too small is unchanged: when `benched` is empty the condition is false and the `creator_seat` fallback applies as before.
...
- **Council ballot budget scales with host load; a slow seat is not a dead seat; evaluator≠creator survives a benched voter (#537).** Three defects fixed. (1) **Budget scaling:** `dispatch_prompt_timed` now computes `factor = clamp(max(load1, load5) / ncpu, 1, 5)` via inline `getloadavg` FFI (no libc dep) and scales the base 40 s ballot timeout to `base × factor`. On S8 (run 0a00ccaa: load1=10.94, load5=15.6, ncpu=14) this yields a 44.6 s budget that seats pi's 40.8 s answer; on S6 (run f39247be) the same shape yields 44.5 s and seats pi's 40.07–40.81 s answers. The `TimedOut` error message now appends `(base × factor, N-min load M / K cpus)` when the factor exceeds 1. (2) **Load-induced bench protection:** a seat whose timeout fires at factor ≥ 2 does not count toward the consecutive-failure bench streak — `record_seat_outcome` returns early for `Healthy` seats under load. Probation is unchanged: the probationary ballot is the readiness contract and load cannot waive it. Genuine timeouts at ratio < 2 still bench normally. (3) **Fail-closed evaluator≠creator (ballot source only):** when a ballot-benched seat empties the evaluator pool, `distribute_units_against_benched` now returns `NoEligibleSeat` instead of silently falling back to the creator seat — a compromised seat must not self-grade its own work. Launcher-bench (source ≠ "ballot") and bench-free small rosters were unchanged at this commit; the full fix covering all bench sources landed in #560. The `PauseReason::RunLevel` gate prompt now names benched seats and the live-vs-configured count so operators understand why human approval was needed.

- **Creator re-dispatch on `request_changes`: operator note now injected at every gate including deliver; full amendment uncapped; `verdictSummaryTrimmed` on `gateEscalated` (#549).** The primary defect: in a wicked-crew governed run, a `request_changes` decision at a **deliver** gate re-dispatched the creator but injected **no** `[review — unit N — requested changes]` context item — the creator never saw the operator's note. The prior fallback read the evaluator's transcript, which for a deliver (tool-executor) unit is empty, so `output.is_empty()` was true and nothing was injected. The same path applied to escalation and run_level gates; all three reach the single `confirm_gate` RequestChanges arm and the single `rewind_to_creator` call site (`src/actor.rs`), so every gate kind is repaired by the same change. The amendment (evaluator findings + operator note) is now stored as `rework_amendment` on `WorkUnit` at rewind time and injected at dispatch from that field — no cap. The unit description carries a bounded marker (≤ `REWORK_MARKER_MAX` chars) BY DESIGN; the full note travels as the injected `[review — unit N — requested changes]` context item, not in the description (FIX-RC2 acceptance amendment, 2026-09-19 22:20Z). Pre-#549 units without the stored field fall back to the old `tail_chars(4096)` path. The evaluator's `findings` field (used for `verdictSummary` in `gateEscalated`) continues to be capped at `EVALUATOR_FINDINGS_CAP` chars; `gateEscalated` now carries `verdictSummaryTrimmed: boolean` (`true` when the cap fired). `UnitDenial` gains `findingsTrimmed: bool` (wire: skipped when false, matches `skip_serializing_if`). `unitContextInjected`'s `outputBytes` per item reflects the full amendment byte length.

- **Repo-checks floor: `unclassified_red_base` classification for zero-ID failures on both sides; node:test TAP and SPEC-reporter parsers (#538).** When the head check exits non-zero and zero failure identifiers are parsed on BOTH head and base (unknown runner output, e.g. node:test without TAP), the floor previously classified as `floor_env_mismatch` (same exit code) or `regression` (different). Neither is correct: without identifiers the failures cannot be compared. The new `unclassified_red_base` classification always denies (fail-closed) and produces an explicit denial note. A node:test / TAP parser for `not ok N - name` lines is added so installer-style repos running node:test get parseable failure identifiers. `# Subtest:` lines are intentionally excluded (they appear for passing subtests too). A node:test SPEC-reporter arm (Node ≥ 23) is also added: `✖ name (Nms)` lines are parsed as failure identifiers using the numeric timing suffix `(Nms)` / `(N.Ns)` as the discriminator — the `✖ failing tests:` summary line and eslint `✖ N problems` lines have no numeric-only timing and are correctly excluded. Indented `  ✖ …` lines from nested suites are handled by the pre-existing `trim_start()`. The existing `floor_env_mismatch` path is unchanged for the `(false, false)` arm where both sides have identifiers that match exactly.
