...
- **The tool-child registry now reports a displaced pgid instead of overwriting in silence (#577); the leader-exit orphan of #575 does not reproduce.** `ElicitationMaps.tool_child_pgroups` is `HashMap<String, u32>` — one pgid per run — and `register_tool_child` overwrote unconditionally while `deregister_tool_child` carefully compare-and-removes. The cardinality is UNCHANGED, because the one-live-tool-child-per-run invariant is real: the actor is the single writer and a run advances one unit at a time, `LaunchRun` refuses a second launch of a live run id (`RunBusy`, via the `in_flight` guard), and `ReassignUnit` drains the registry with `take_tool_child_pgroups` and kills the group BEFORE dispatching the replacement. A wider data structure would be storage for a state the engine does not produce; what was missing was the assertion, not the capacity. `register_tool_child` is now `#[must_use]` and returns `Option<u32>` — the pgid it displaced, which by that invariant is always `None` — and the caller (`on_spawn`) logs loudly when it is not: the displaced group can no longer be killed by the actor, though its own worker thread still kills it, so this is lost REACH, not a lost process. Deliberately not a panic: turning a recoverable loss of reach into a worker-thread panic while holding the lifecycle mutex would be worse than the condition it reports. `take_tool_child_pgroups`'s `Vec` is documented as 0-or-1 by construction rather than as evidence that several groups are tracked. **On #575** (a tool child's pgroup deregistered when its LEADER exits, orphaning backgrounded work): probed against the real `run_tool_cmd`, three runs, and the backgrounded process is already dead when the call returns — the `Ok(true)` arm kills the whole group before collecting the status, while the un-reaped zombie leader still reserves the pid that is the group id, so the group kill lands before `on_done` deregisters anything. The premise does not hold on unix and nothing is changed for it; a regression test (`a_natural_leader_exit_still_takes_everything_it_backgrounded_with_it`) now pins that ordering, since the group kill on an already-exited child reads like a redundant line.
