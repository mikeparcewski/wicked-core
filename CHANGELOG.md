...
- **A campaign Reject that cannot cancel its run now ERRORS instead of reporting success (#574).** `campaign::confirm_gate`'s `CampaignGateDecision::Reject` arm discarded the `Result` of the inner `actor::confirm_gate` call with `let _ =`. `actor::confirm_gate` refuses unless `session.status == AwaitingHuman`, and the campaign's `node_status` can disagree with the session's status — nothing reconciles the node when the run leaves `AwaitingHuman` by a non-campaign route (e.g. the run-level `Core::confirm_gate` answered the gate directly). The outer node guard then passed while the inner session guard bailed, the error was dropped, and the operator was told `Ok(<campaign status>)` while the run kept executing: no `RunCancelled`, child still alive. The inner error is now propagated, wrapped with the campaign id, node id and run id (`campaign <c> node <n>: reject could not cancel run <r>: <cause>`); the campaign state is left untouched so the node stays `AwaitingHuman` and the reject can be re-issued once the state is reconciled. Behaviour change for API callers: `ConfirmCampaignGate` with `Reject` can now return `Err` where it previously returned `Ok`. The two sibling call sites are unchanged — the Approve/resume path already used `?`, and `denial_gate: auto_reject` still logs and HOLDS. The underlying node-status drift (the second defect noted in #574) is NOT fixed here; only the swallowed error is.
