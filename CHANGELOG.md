...
- **`Core.busEmit` + `Core.busRead`: crew writes and reads the bus through the engine (wicked-core#631).** The engine is the one writer and the one SQLite library on its bus, so crew can drop its in-daemon better-sqlite3 and its child-process writer (crew#679/#680). **`Core::bus_emit(event_json)`** (core-ts `busEmit(eventJson): Promise<number>`) takes an event as wicked-bus `emit()` takes it — `{event_type, domain, subdomain?, payload, idempotency_key?, producer_id?, ttl_hours?}` — validates it as wicked-bus `validateEvent` does (`WB-001 …`: the `wicked.<seg>(.<seg>)*` type in at most 128 chars, domain/subdomain at most 64, an object payload within `config.max_payload_bytes`; a field this emit does not write is refused, not dropped) and writes it through `BusDb::emit`: the same row and two-timer TTL, and a key already on the bus resolves to the existing row's id (wicked-bus raises WB-002 there, which crew treats as success). It is not wicked-bus `emit()` in full: no schema registry (size policy, CAS offload, JSON Schema) runs, and no causality field (`correlation_id`, `session_id`, `parent_event_id`) is taken from the environment. **`Core::bus_read(after_id, limit, type_prefix?, include_expired?)`** (core-ts `busRead(afterId, limit, typePrefix?, includeExpired?): Promise<string>`) answers `{next, rows}`: live rows (`expires_at` in the future; every row with `include_expired`, for history readers) after the cursor, oldest first, filtered by a literal type prefix in SQL, at most `limit` (capped at 1000), each a whole `events` row with `payload` parsed; `next` is the last row's id on a full page, else the bus tail read before the rows (so a short page never rescans what the prefix skipped), and `0` when the cursor is past the tail (the file was replaced and its ids restarted); `limit` 0 answers the tail alone. Both run on the caller's thread (the libuv pool from JS) over the process-wide `BusDb::shared` connection, never through the actor, and error when the core has no bus. `BusEmit` gains `producer_id`, and opening a bus adds wicked-bus migration 3's seven `events` columns (`parent_event_id`, `session_id`, `correlation_id`, `producer_id`, `origin_node_id`, `registry_schema_version`, `payload_cas_sha`), each only if missing and in that migration's order, so an engine-created file has a wicked-bus-created file's column order (wicked-bus's archive sweep copies rows by position); a `duplicate column name` from a racing JS migrator is success.
