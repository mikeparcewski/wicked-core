...
- **Creator re-dispatch on `request_changes`: full operator amendment delivered uncapped; `verdictSummaryTrimmed` on `gateEscalated` (#549).** When a deliver-gate evaluator's `request_changes` decision re-dispatches the creator, the creator previously received only a ~4 KB tail of the evaluator transcript as prior context (capped by `tail_chars`). The amendment (evaluator findings + operator note) is now stored as `rework_amendment` on `WorkUnit` at rewind time and injected at dispatch from that field — no cap, no re-read of the transcript. Pre-#549 units without the stored field fall back to the old `tail_chars(4096)` path. The evaluator's `findings` field (used for `verdictSummary` in `gateEscalated`) continues to be capped at `EVALUATOR_FINDINGS_CAP` chars; `gateEscalated` now carries `verdictSummaryTrimmed: boolean` (`true` when the cap fired). `UnitDenial` gains `findingsTrimmed: bool` (wire: skipped when false, matches `skip_serializing_if`). `unitContextInjected`'s `outputBytes` per item reflects the full amendment byte length.
