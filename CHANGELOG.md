...
- **Gate hook: INDEPENDENT REVIEW defects fixed; command-string scanners retired (#542, #548).**
  (1) **Windows path detection (CI red):** `is_abs_path_token` —
  `!tok.starts_with("//") && Path::new(tok).is_absolute()` — covers Windows drive letters
  (`C:\…`), `\\?\` long paths, and UNC forms on all platforms; `:` removed from the split
  delimiter so drive letters survive tokenisation.
  ...
  `>>`, python heredocs referencing `/etc/hosts` in comments, etc.). A Creator unit writing
  outside its tree through a quoted interpreter string or heredoc body is caught by the
  post-hoc witness after the call, not blocked pre-call; see wicked-core#548 for OS-level
  enforcement.
...
- **Gate hook: ReadOnly write fence for `touch`, inline interpreters, and post-hoc witness
  ...
  **by program word** when the write posture is ReadOnly — no argument parsing or code-string
  ...
  quoted interpreter string or heredoc is caught by the post-hoc witness after the call, not
  blocked pre-call; see wicked-core#548 for OS-level enforcement. The post-hoc witness stores a
...
- **Gate hook: close worktree boundary misses for `git -C <other-path>` and `cd <other-path> &&
  ...
  `cd` commands with no following write are left to the install fence. A Creator unit writing
  outside its tree through a `cd` combined with a quoted interpreter string (no statically
  resolvable target) is caught by the post-hoc witness after the call; see wicked-core#548.
