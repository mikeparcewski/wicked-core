...
- **T0 bus handoff, engine half (DES-TEAMING-002 T0, rev 11).** The engine can now be handed a bus on every boot without changing what a default daemon does. **(1)** The gate judge's bus round-trip (`wicked.gate.eval.requested`, 180 s fail-closed) switches on exec mediation's own setting, `WICKED_BUS_EXEC` plus `WICKED_BUS_DB`, and never on `WICKED_BUS_DB` alone; before this, the variable by itself rerouted every evidence-floor-pinned judge to a responder no product runs. **(2)** The actor never opens the bus. The launch bridge opens it and takes its start point on its own thread (a time floor taken in `Core::spawn*`, `BusDb::floor_before`), and exec mediation's init runs on a bus thread, so a locked bus no longer stalls the first command by the 5 s busy timeout. **(3)** One connection per bus file per process, never closed (`BusDb::shared`; `BusDb` clones share it), because crew's better-sqlite3 seams share the file in the same process and any close drops the process's POSIX locks (the F-E2E-021 class). A source-guard test fails the build on a production `BusDb::open`. **core-ts:** `Core.busConnectionStats(path)` reports the one connection; crew keys the handoff on its presence.
