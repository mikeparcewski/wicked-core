...
| **Validator execution hardening** — env-clear (secrets dropped) + pinned cwd + bounded process-group-kill timeout on every run; a real OS sandbox (macOS `sandbox-exec` / Linux `bwrap`) when present: network-denied + curated secret-dir read-block; the applied `SandboxLevel` is reported, never overclaimed. **Honest limits:** best-effort (not a hard jail); Windows/no-tool boxes run the floor only; adversarially reviewed (8 findings incl. a file-exfil channel, all fixed) | ✅ built (best-effort) |
| **Distinct council seat for the agent judge** — the agent validator runs under a seat identity-distinct from **both** the deterministic-validator author **and** the work author (no self-grading), by normalized binary identity; single-runner fallback when the roster is too small. Adversarial review caught a self-grade hole (judge excluded the wrong author); fixed + dispatch-proven | ✅ built |
| napi → **studio UI** wiring (the addon exists; the surface consumes it) | 📐 design spike — `../wicked-studio/.product/DES-STUDIO-NAPI-001.md` (grounded: studio is Rust eframe today; the SPA is built from zero) |
| Council-assigns-skill-at-runtime | 📐 design spike — `.product/DES-COUNCIL-SKILL-001.md` (grounded skill ranking over the real installed inventory; names the council-prompt-is-a-literal constraint + 2 build options) |
